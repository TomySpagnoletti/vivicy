...
- `factory/` — standalone Node ESM tooling. The actual modules: the issue extractor (`extract-issues.mjs`), the dev-loop orchestrator (`dev-loop.mjs`) and resumable supervisor (`dev-loop-supervised.mjs`), the status probe (`dev-status.mjs`), the rehearsal harness (`dev-rehearsal.mjs`), the per-issue progress ledger (`progress-ledger.mjs` — the single source of truth for progress, written mechanically by the orchestrator), the polyglot gate config (`project-config.mjs`, reading the target's `vivicy.json`), the agent-leg spawn + per-leg timeout infra (`agent-spawn.mjs`, `leg-timeout.mjs`, `leg-supervisor.mjs`), the documentation-baseline lock (`doc-baseline.mjs`), the semantic-extraction and traceability gates (`semantic-extraction-check.mjs`, `traceability-check.mjs`), the four agent prompts (`prompts/`), the lean-target scaffold templates (`templates/`), and the architecture-map viewer-data generator (`generate-viewer-data.ts`). Plain `node`, no Next coupling. It has its own `factory/tsconfig.json` and a Node `--test` suite; it is excluded from the Next app's TypeScript and ESLint configs on purpose. There is **no progress MCP and no lifecycle-hook seam** — agents never report progress; the orchestrator owns the ledger.
