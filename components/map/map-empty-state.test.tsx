...
describe("MapEmptyState — guidance per empty reason", () => {
  ...
  test("empty_canonical renders the arrow instruction as two deliberate centered lines — no card, border, icon, title, or buttons", () => {
    renderEmptyState(<MapEmptyState reason="empty_canonical" onExtract={vi.fn()} onImported={vi.fn()} />)
    const sentence = document.querySelector<HTMLParagraphElement>('[data-empty-reason="empty_canonical"]')!
    expect(sentence.tagName).toBe("P")
    expect(sentence).toHaveClass("text-center")
    expect(sentence).toHaveClass("text-muted-foreground")
    expect(sentence.querySelector("br")).not.toBeNull()
    expect(sentence.textContent?.startsWith("←")).toBe(true)
    expect(sentence.textContent).toContain("Talk to Vivi to get grilled")
    expect(sentence.textContent).toContain("Your answers turn into the spec.")
