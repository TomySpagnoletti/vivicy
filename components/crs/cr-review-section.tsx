    ...
    async (id: string, decision: "approved" | "rejected") => {
      ...
      try {
        ...
        if (!res.ok || body.ok === false) {
          ...
          toast.error(`${id} ${decision === "approved" ? "approved but apply blocked" : "decision failed"}`, {
            description: body.summary ?? body.error ?? `HTTP ${res.status}`,
          })
        } else {
          toast.success(`${id} ${decision}`, { description: body.summary })
        }
      } catch (error) {
        toast.error(`${id} decision failed`, {
  ...
  return (
    <div className="border-b border-border px-4 py-3">
      <div className="mb-2 flex items-center gap-1.5">
        ...
        <span className="text-xs font-medium text-foreground">Change requests awaiting your decision</span>
        ...
        {pending.map((cr) => (
          <li key={cr.id} className="flex flex-col gap-1.5 rounded-md border border-border bg-card p-2.5 text-xs">
            <div className="flex items-center gap-1.5">
              ...
              {cr.source ? (
                <span className="ml-auto shrink-0 text-[10px] text-muted-foreground">via {cr.source}</span>
              ) : null}
            </div>
...
function ConfirmDecision({
  ...
  return (
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>{approve ? `Approve ${id}?` : `Reject ${id}?`}</AlertDialogTitle>
          <AlertDialogDescription>
            {approve
              ? "Approving folds this change into the canonical: apply → re-freeze the baseline → re-extract the impacted issues. This runs autonomously and changes the frozen spec."
              : "Rejecting closes this change request. The product intention is left unchanged."}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel>Cancel</AlertDialogCancel>
          <AlertDialogAction onClick={onConfirm}>{approve ? "Approve" : "Reject"}</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  )
