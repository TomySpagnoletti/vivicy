...
test.describe("sidebar widths (24rem / 36rem)", () => {
  test("the toggle cycles peek=24rem -> wide=36rem -> closed", async ({ page }, testInfo) => {
    await page.goto("/")
    ...
    test.skip(isMobileProject(testInfo), "Mobile uses a single-width off-canvas Sheet; the width cycle is desktop-only.")
...
test.describe("legend lives in the sidebar (collapsed by default)", () => {
  test("the legend section is present, collapsed, and expands", async ({ page }, testInfo) => {
    await page.goto("/")
    await expect(page.locator(".react-flow__node").first()).toBeVisible({
      timeout: 30_000,
    })
  ...
  test("the legend lives in the fixed footer region, above the quota footer", async ({ page }, testInfo) => {
    await page.goto("/")
    await expect(page.locator(".react-flow__node").first()).toBeVisible({
      timeout: 30_000,
    })
...
test.describe("minimap is present and non-empty", () => {
  test("renders node rects filled with real (non-white) color", async ({ page }) => {
    await page.goto("/")
    await expect(page.locator(".react-flow__node").first()).toBeVisible({
      timeout: 30_000,
    })
    ...
    const someColored = paints.some((p) => p.stroke !== "rgb(255, 255, 255)" && p.stroke !== "rgba(0, 0, 0, 0)")
