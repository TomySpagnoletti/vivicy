...
function writeIssueIndex(dir: string, ids: string[]): void {
  writeFileSync(join(dir, ".vivicy/development/issue-index.json"), JSON.stringify({ issues: ids.map((id) => ({ id, graph_refs: [] })) }))
}
...
describe("crs --json", () => {
  test("lists well-formed CRs, skipping template + readme", () => {
    ...
    assert.equal(r.code, 0)
    assert.deepEqual(
      r.json.crs.map((c: { id: string }) => c.id),
      ["CR-0001", "CR-0002"]
    )
...
describe("spawning verbs against a stub factory", () => {
  ...
  before(() => {
    ...
    writeFileSync(
      join(stubFactory, "dev-status.ts"),
      "console.log(JSON.stringify({ verdict: 'NOT STARTED', issues_total: 0, issues_done: 0, active: [], gates: { pass: 0, fail: 0 } }));\nprocess.exit(0);\n"
    )
  })
  after(() => rmSync(stubFactory, { recursive: true, force: true }))
