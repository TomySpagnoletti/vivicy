...
export const MAP_REVIEW_LENSES: MapReviewLens[] = [
  {
    key: "product",
    focus:
      "Product and real-world workflow: the graph reflects how the product actually behaves end to end; a human can challenge the system from left to right without reading every doc.",
  },
  {
    key: "architecture",
    focus:
      "Architecture and runtime: every always-on service, worker runtime, local wrapper, queue, materialization step, and provider dependency that matters to implementation is represented; no fabricated or missing runtime component.",
  },
  {
    key: "security",
    focus:
      "Security and secrets: every security, credential, tenancy, authorization, network, and provider boundary is explicit; no edge implies a bypass the docs forbid; read-only inspection edges are labeled read-only.",
  },
  {
    key: "data-ownership",
    focus:
      "Data and source-of-truth ownership: every durable state has exactly ONE owning node; the same state is never stored in several nodes; no duplicate authority.",
  },
  {
    key: "protocol",
    focus:
      "Protocol and API boundaries: every worker/service communication path uses the one documented protocol; no second path or shadow protocol for the same relationship; edge identity (from+to+relation+protocol) is meaningful.",
  },
  {
    key: "infrastructure",
    focus:
      "Infrastructure and provider implementation: provider-specific nodes sit behind a declared provider-neutral boundary when the docs require abstraction; provider names do not leak into product-level nodes.",
  },
  {
    key: "observability",
    focus:
      "Observability and cost: audit, observability, cost, and security controls are represented and sit in a supporting band, not on the main production path.",
  },
  {
    key: "dev-traceability",
    focus:
...
export function mapReviewLensContext({
  lens,
  manifestPath,
  baselineId,
}: {
...
export async function runMapReview({
  repoRoot,
  manifestPath,
  baselineId,
  cfg,
  ...
  const legs = await Promise.all(lenses.map((lens) => spawnLens({ repoRoot, manifestPath, baselineId, cfg, attempt, lens })))
