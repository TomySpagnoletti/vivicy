...
//   1. issue.gate_command   — a per-issue override read in dev-loop.mjs.
 ...
 */
export function validateGateCommand(value, source = PROJECT_CONFIG_FILENAME) {
 ...
 */
export function loadProjectConfig(targetRoot) {
  ...
  if (existsSync(pkgPath)) {
    ...
    if (pkg && typeof pkg === "object" && pkg.vivicy && typeof pkg.vivicy === "object") {
...
function parseConfig(text, source) {
  ...
  try {
    ...
    throw new ProjectConfigError(`${source}: invalid JSON — ${error?.message ?? error}`, "invalid_json");
  }
...
function normalizeConfig(raw, source) {
  ...
  return { gateCommand: validateGateCommand(raw.gateCommand, source) };
}
 ...
 */
export function resolveGateCommand({ issue, targetRoot, explicitDefault } = {}) {
