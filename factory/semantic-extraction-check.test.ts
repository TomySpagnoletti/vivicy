...
test("real repo artifacts: placeholder index passes without writing reports", () => {
  ...
  try {
    ...
    assert.ok(!existsSync(resolve(target.root, ".vivicy/requirements/source-map.json")), "no source map written in placeholder mode")
  } finally {
    target.cleanup()
  }
})
...
const EXAMPLE_EXCLUSION = {
  end: 10,
  file: SAMPLE_DOC_PATH,
  note: "fence body is illustrative",
  reason_class: "example_illustration",
  start: 10,
}
...
test("valid refs with full coverage pass and write the three reports", () => {
  ...
  try {
    ...
    const coverageJson = JSON.parse(
      readFileSync(resolve(fixture.root, ".vivicy/requirements/coverage-report.json"), "utf8")
    ) as CoverageReport
...
test("a well-formed Proofs block passes and never feeds line coverage (a proof points at an obligation, it never carries one)", () => {
  ...
  try {
    ...
    try {
      assert.deepEqual(result.coverage!.totals, bare.run().coverage!.totals, "declaring proofs changes no coverage number")
    } finally {
      bare.cleanup()
    }
  } finally {
...
test("a malformed Proofs block fails at extraction, where the extractor can still fix it", () => {
  const cases: Array<[string[], RegExp]> = [
    ...
    [
      ["- id: shot", "  class: run_log", "  evidences:", "  - .vivicy/canonical/absent.md:5"],
      /proof "shot" evidence ref .* outside the pinned manifest corpus/,
    ],
...
test("requirement ref beyond the file's line count fails", () => {
  ...
  try {
    ...
    assert.equal(result.exitCode, 1)
    assert.ok(
      result.errors.some((error) => /out of range/.test(error)),
      result.errors.join("\n")
    )
    assert.equal(result.reportsWritten, false, "structural failures write no reports")
  } finally {
...
test("requirement ref outside the canonical grammar fails", () => {
  ...
  try {
    ...
    assert.equal(result.exitCode, 1)
    assert.ok(
      result.errors.some((error) => /source ref grammar/.test(error)),
      result.errors.join("\n")
    )
  } finally {
    fixture.cleanup()
...
test("an uncovered canonical line fails the gate and is listed in the report", () => {
  ...
  try {
    ...
    assert.equal(result.exitCode, 1)
    assert.ok(
      result.errors.some((error) => /UNCOVERED/.test(error)),
      result.errors.join("\n")
    )
...
test("llm_extraction_in_progress tolerates uncovered lines; --strict escalates", () => {
  ...
  try {
    ...
    assert.ok(
      strict.errors.some((error) => /UNCOVERED/.test(error)),
      strict.errors.join("\n")
    )
  } finally {
    fixture.cleanup()
...
test("a depends_on cycle fails the gate", () => {
  ...
  try {
    ...
    assert.equal(result.exitCode, 1)
    assert.ok(
      result.errors.some((error) => /cycle detected/.test(error)),
      result.errors.join("\n")
    )
  } finally {
    fixture.cleanup()
...
test("depends_on referencing an unknown issue id fails", () => {
  ...
  try {
    ...
    assert.equal(result.exitCode, 1)
    assert.ok(
      result.errors.some((error) => /unknown issue id/.test(error)),
      result.errors.join("\n")
    )
  } finally {
    fixture.cleanup()
...
test("index pin mismatch against the manifest fails even in placeholder mode", () => {
  ...
  try {
    ...
    assert.equal(result.exitCode, 1)
    assert.ok(
      result.errors.some((error) => /pin mismatch.*manifest_hash/.test(error)),
      result.errors.join("\n")
    )
  } finally {
    fixture.cleanup()
...
test("a line both covered and excluded warns, covered wins; --strict escalates", () => {
  ...
  try {
    ...
    assert.equal(lenient.exitCode, 0)
    assert.ok(
      lenient.warnings.some((warning) => /covered wins/.test(warning)),
      lenient.warnings.join("\n")
    )
...
test("exclusion ref out of the doc's line range fails", () => {
  ...
  try {
    ...
    assert.equal(result.exitCode, 1)
    assert.ok(
      result.errors.some((error) => /exclusions\[0\].*out of range/.test(error)),
      result.errors.join("\n")
    )
  } finally {
    fixture.cleanup()
