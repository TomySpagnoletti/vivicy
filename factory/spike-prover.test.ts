...
function writeSpike(
  filename: string,
  {
    ...
    gated_by,
    blocks,
  }: { status?: string; reqId?: string; gated_by?: string; blocks?: string } = {}
): { file: string; path: string; gate_id: string } {
...
function fakeSpikeVerifier(
  agreeByGate: Record<string, boolean | "__NO_REPORT__"> = {},
  { onAttempt }: { onAttempt?: (ctx: any) => boolean | "__NO_REPORT__" | null } = {}
) {
...
describe("runSpikeProving — agree + verified", () => {
  ...
  it("emits spike_proof_started/completed through the injected recordEvent sink", async () => {
    ...
    assert.ok(
      (progressRoles as readonly string[]).includes(events[0].role as string),
      "spike_proof_started role is a declared progress role"
    )
    assert.ok(
      (progressRoles as readonly string[]).includes(events[1].role as string),
      "spike_proof_completed role is a declared progress role"
    )
    assert.ok(
      (progressEventTypes as readonly string[]).includes(events[0].event_type as string) &&
        (progressEventTypes as readonly string[]).includes(events[1].event_type as string),
...
describe("spikeProvingCrNotification", () => {
  it("stays silent when no CR was drafted and warns naming the ids when one was", () => {
    ...
    const note = spikeProvingCrNotification([
      { file: "a.md", id: "CR-0001" },
      { file: "b.md", id: "CR-0002" },
    ])
...
describe("runSpikeProving — agree + failed drafts a Change Request", () => {
  ...
  it("flips the spike to failed and writes a CR whose frontmatter passes change-control", async () => {
    ...
    assert.match(
      ...
      "the CR records the spike gate_id on affected_verification_gates"
    )
...
describe("runSpikeProving — topological ordering of the gated_by graph", () => {
  ...
  it("skips a spike whose gate FAILED this run (never proves on an unproven foundation)", async () => {
    ...
    assert.deepEqual(
      proofCalls.map((c) => c.gate_id),
      [b.gate_id],
      "the dependent's legs never ran"
    )
    assert.deepEqual(
      result.failed.map((f) => f.gate_id),
      [b.gate_id]
    ...
    assert.deepEqual(
      result.skipped.map((s2) => s2.gate_id),
      [a.gate_id],
