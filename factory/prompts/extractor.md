...
4. **Vertical issues** — `.vivicy/development/issues/<issue_id>.md`, one file per issue. An `issue_id` is EXACTLY `ISSUE-` followed by four digits, zero-padded and unique across the corpus (`ISSUE-0001`, `ISSUE-0002`, … — never a bare number, an abbreviation, or a slug); the gate refuses any other shape. Each file follows the **Issue file shape** (see "## File shapes") exactly (keep the section headings verbatim; the checker finds the Traceability block by its heading). Each issue is the smallest vertical slice that still delivers real, testable behavior end to end (authored to the **granularity law** under Discipline). The `## Traceability` block's lists (`issue_id`, `graph_refs`, `requirement_ids`, `source_line_refs`, `depends_on`, `spike_gates`, `verification_gate_ids`) MUST match the index entry exactly. `source_line_refs` use the `.vivicy/canonical/<file>.md:<start>[-<end>]` grammar and point only into the pinned corpus. Write deterministic Scope/Out-of-Scope/Verification prose — no "if needed", "simplify where appropriate", or vague wording.
...
## Run command (only if the canonical STATES it)

`vivicy.json#runCommand` is the command that starts the built product for the owner to run and use — the "how it runs and ships" story, distinct from the verification gate. The scaffold leaves it as the sentinel `null` (not yet established). If — and ONLY if — the frozen canonical's run-and-ship area EXPLICITLY states the command that runs the product (e.g. a doc line that says the product is served with `npm run dev`, `flask run`, `go run ./...`, …), record it so the orchestrator can establish it: write `{ "runCommand": "<the exact command the canonical states>" }` to `.vivicy/development/reports/extraction-run-command.json`. Never guess, never infer from the stack, never invent a placeholder, and never point it at the test runner — if the canonical does not state a run command, do NOT write this file; the sentinel stays and the stack-setup issue's implementer establishes it. Do not touch `vivicy.json` yourself.
...
### Issue file — `.vivicy/development/issues/<issue_id>.md` (e.g. `ISSUE-0001.md`)
...
- **Granularity law — the smallest end-to-end-provable slice.** Order issues so dependencies form a DAG, and make each issue the SMALLEST slice that still proves ONE behavior end to end: one obligation cluster, the minimal file surface, gate-provable ALONE by its own `verification_gate_ids` with no sibling issue in flight. Respect the floor too — a slice too thin to prove a behavior on its own is also wrong, since it only pushes the drift into the seams between issues. A spec area big enough to yield one sprawling issue is DECOMPOSED into an ordered chain of such slices instead of a single omnibus, each independently gateable, the ordering made explicit through each issue's `depends_on` in the index. An issue's `verification_gate_ids` name real gates (e.g. `gate:test:<slug>`), not free-text.
