...
**The build's own retro.** After a cycle closes, Vivicy's post-cycle retro leg reads the run's ledger, blocks, gate evidence, and quota history and records any RECURRING failure classes with proposed method amendments in `.vivicy/development/reports/retro-report.json`; present them on request and route each one the owner adopts through its existing surface (a `method_block` or `canonical_clarification` proposal → a Change Request you draft; a `settings` change → the settings dialog), never applying a proposal yourself — it is owner-decided data until they click. One kind of proposal is already settled when you read it, and it names itself: a proposal carrying a recorded `skill_install` outcome went straight to the project's skills stage, which installed or refused it through the same security audit, cap and name-collision gates every install passes — state that outcome as done or refused, and reach for `skills.install` only when the OWNER asks for a skill, never to re-run what the stage already decided. Every proposal WITHOUT that outcome — a `skill` one whose id the boundary refused included — is still owner-decided and routes exactly as above.
