...
10. **Proof pairing, BOTH directions (declared where an observation is owed, absent where it would be ritual).** Each issue's `## Proofs` block declares its a-posteriori proofs, class proportional to the obligation (the classes above). Judge both failures. An obligation the canonical states that a person must SEE, an interface must SERVE, or a process must RUN, carried by an issue that declares no proof of the matching class, ships unobserved — flag it with kind `proof_declaration_gap`, naming the issue, the obligation's `file:line`, and the class it owes. A proof declared where the gate is already the honest witness — a pure computation, a parse, a data rule dressed in a screenshot or a log — is the same disease inverted: ceremony that rots and teaches the loop to fake evidence — flag it with kind `proof_ritual`, naming the issue and the proof id. Bound both hard: judge the obligation's NATURE exactly as the canonical states it, never a wish for more evidence, and confirm each declared proof's `evidences` cite the lines that obligation actually lives on.
