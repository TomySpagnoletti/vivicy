...
function fixture({ issues, requirements, spikes }: { issues: unknown[]; requirements?: unknown[]; spikes?: Record<string, string> }) {
...
test("fails when an issue references an unknown requirement", () => {
  ...
  try {
    ...
    assert.ok(
      r.errors.some((e) => /REQ-GHOST-999/.test(e)),
      r.errors.join("\n")
    )
  } finally {
    f.cleanup()
...
test("fails when an MVP requirement is covered by no issue", () => {
  ...
  try {
    ...
    assert.equal(r.exitCode, 1)
    assert.ok(
      r.errors.some((e) => /REQ-B-002/.test(e)),
      r.errors.join("\n")
    )
  } finally {
    f.cleanup()
...
test("requires the catalog once issues exist", () => {
  ...
  try {
    ...
    assert.equal(r.exitCode, 1)
    assert.ok(
      r.errors.some((e) => /catalog_required/.test(e)),
      r.errors.join("\n")
    )
  } finally {
    f.cleanup()
...
const spikeMd = ({ slug, status = "pending", reqIds = "REQ-A-001" }: { slug: string; status?: string; reqIds?: string }) =>
  [
    "# S01 - X",
    "## Traceability",
    "```text",
...
test("spike gating: an issue spike_gate with no spike file fails", () => {
  ...
  try {
    ...
    assert.equal(r.exitCode, 1)
    assert.ok(
      r.errors.some((e) => /issue_spike_resolves/.test(e)),
      r.errors.join("\n")
    )
  } finally {
    f.cleanup()
...
test("spike gating: a spike still carrying the placeholder requirement_ids fails", () => {
  ...
  try {
    ...
    assert.equal(r.exitCode, 1)
    assert.ok(
      r.errors.some((e) => /spike_requirement_backfilled/.test(e)),
      r.errors.join("\n")
    )
  } finally {
    f.cleanup()
...
test("spike gating: a spike requirement_id that is not in the catalog fails", () => {
  ...
  try {
    ...
    assert.equal(r.exitCode, 1)
    assert.ok(
      r.errors.some((e) => /spike_requirement_resolves/.test(e)),
      r.errors.join("\n")
    )
  } finally {
    f.cleanup()
...
test("spike gating: a must_verify_with_spike requirement with no spike to gate it fails", () => {
  ...
  try {
    ...
    assert.equal(r.exitCode, 1)
    assert.ok(
      r.errors.some((e) => /spike_requirement_gated/.test(e)),
      r.errors.join("\n")
    )
  } finally {
    f.cleanup()
