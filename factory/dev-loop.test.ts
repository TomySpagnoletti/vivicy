...
// as a side effect, before dev-loop.ts (imported below) binds its target root at
// module load. See test-target-root.ts for why import order matters here.
...
// Vivicy is a STANDALONE factory: dev-loop.ts binds its target root from
...
test("runLoop: a leg that keeps TIMING OUT is retried, then issue_blocked with the timeout reason (never hangs)", () => {
  ...
  // back as a leg-timeout.ts KILL (status 124, timedOut:true) every attempt, and
...
// One recorded concurrency-window event from a fake parallel leg.
