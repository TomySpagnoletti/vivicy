...
export function validateMap(input: ArchitectureMap): void {
  ...
  for (const node of input.nodes) {
    ...
    if (node.status !== "not_started") {
      throw new Error(
        `Target map node ${node.id} must keep status not_started; live progress belongs in .vivicy/development/progress-ledger.json`
      )
    }
...
export function assertYamlSourceStyle(source: string): void {
  source.split(/\r?\n/).forEach((raw, index) => {
    ...
    if (trimmed.startsWith("#")) {
      if (indent >= 2) {
        throw new Error(
          `architecture-map.yml line ${lineNumber}: comments must not be interleaved inside records or lists; put comments at column 0.`
        )
      }
      return
    }
...
function readIssues(input: unknown, graphRefs: Set<string>, verificationGateRefGrammar: string): DevelopmentIssue[] {
  ...
  if (input.issues.length > 0) {
    ...
    if (issueManifest.status !== "frozen") {
      throw new Error(
        `Issue index must pin a frozen baseline manifest; ${String(input.manifest_path)} status is ${String(issueManifest.status)}`
      )
    }
    for (const [field, indexValue, manifestValue] of [
      ...
      if (indexValue !== manifestValue) {
        throw new Error(
          `Issue index ${field} (${String(indexValue)}) does not match the frozen baseline manifest (${String(manifestValue)}); the issue index must pin the active frozen baseline`
        )
      }
    }
  }
...
function readCoverageSummary(input: unknown): DevelopmentOverlay["coverage_summary"] {
  ...
  return {
    ...
    requirement_linked_doc_lines: requiredNumber(summary.requirement_linked_doc_lines, "coverage_summary.requirement_linked_doc_lines"),
...
function validateNodeShape(node: NodeSpec): void {
  ...
  if ("evidence_refs" in node) {
    throw new Error(
      `Target map node ${node.id} must not store evidence_refs; live evidence belongs in .vivicy/development/progress-ledger.json`
    )
  }
}
...
function validateSourceBaseline(sourceBaseline: SourceBaseline): void {
  ...
  if (frozenManifest.status !== "frozen") {
    throw new Error(
      `source_baseline must pin a frozen baseline manifest; ${sourceBaseline.manifest_path} status is ${String(frozenManifest.status)}`
    )
  }
  ...
  for (const [field, mapValue, manifestValue] of baselinePins) {
    if (mapValue !== manifestValue) {
      throw new Error(
        `source_baseline.${field} (${String(mapValue)}) does not match the frozen baseline manifest (${String(manifestValue)}); the architecture map must pin the active frozen baseline`
      )
    }
  }
}
...
export function updateGeneratedDate(source: string, date: string): string {
  ...
  if (updatedKeyCount !== 1) {
    throw new Error(`Architecture map metadata rewrite produced ${updatedKeyCount} top-level "updated:" lines (expected exactly 1)`)
  }
  return next
}
...
export function parseArchitectureMap(source: string): ArchitectureMap {
  ...
  for (const rawLine of lines) {
    ...
    if (
      (section === "rules" ||
        section === "kind_taxonomy" ||
        section === "kind_definitions" ||
        section === "flow_classes" ||
        section === "high_risk_kinds") &&
      indent === 2 &&
      line.startsWith("- ")
    ) {
