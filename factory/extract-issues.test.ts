...
// The context objects the extraction orchestrator hands the injected leg seams. The
// fakes read a subset; typed structurally so the test doubles stay honest without
// importing the orchestrator's private arg interfaces.
...
// The status object the orchestrator hands emitStatus; the tests inspect these fields.
 ...
 * spike-prover.test.ts; the proof that S3 runs BEFORE the freeze lives below.
 */
...
function stubSeams(extra: Record<string, unknown> = {}) {
  ...
  return {
    ...
    // spike-prover.test.ts. A test overrides it via `extra` to exercise the ordering.
...
describe("extractIssues — bounded retries / blocked (deterministic)", () => {
  ...
  it("a TIMED-OUT extractor leg is retried, then extraction_blocked with the timeout reason (never hangs)", async () => {
    seedInputs(temp);
    // The fake extractor mimics a leg that leg-timeout.ts KILLED: it authors
