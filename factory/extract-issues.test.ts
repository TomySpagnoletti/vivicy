...
import {
  ...
  formatFixContext,
  formatMapError,
  recordExtractedGateCommand,
  recordExtractedRunCommand,
  resolveFreezeVersion,
} from "./extract-issues.ts"
...
describe("extractIssues — deterministic fix loop", () => {
  it("re-prompts with the exact check output when attempt 1 fails, then goes green on attempt 2", async () => {
    seedInputs(temp)
    const { spawnExtractor, calls } = fakeAgent([(ctx) => writeInvalidCorpus(ctx.repoRoot), (ctx) => writeValidCorpus(ctx.repoRoot)])
    const { spawnVerifier, calls: verifyCalls } = alwaysFaithfulVerifier()
    const seams = stubSeams()
...
describe("extractIssues — fidelity fix loop (the independent verifier)", () => {
  it("verifier returns problems on attempt 1, extractor fixes, attempt 2 faithful -> green", async () => {
    seedInputs(temp)
    const { spawnExtractor, calls } = fakeAgent([(ctx) => writeValidCorpus(ctx.repoRoot), (ctx) => writeValidCorpus(ctx.repoRoot)])
    const { spawnVerifier, calls: verifyCalls } = fakeVerifier([
      {
        ...
        problems: [
          {
            issue: "ISSUE-0003",
            kind: "invented_requirement",
            detail: "ISSUE-0003 invents a rate-limit obligation the cited lines do not state.",
          },
        ],
      },
...
describe("extractIssues — map-generation GATE (the live-run fragility this fix closes)", () => {
  ...
  it("GREEN requires deterministic + map + fidelity all clean (proven by withholding each one)", async () => {
    ...
    const fidRed = await run({
      corpus: writeValidCorpus,
      verdict: { faithful: false, problems: [{ issue: "ISSUE-0001", kind: "scope_drift", detail: "broadens scope" }] },
      map: MAP_OK,
    })
  ...
  it("minor-bumps the HIGHEST baseline version ever cut, whatever the statuses (superseded/draft included)", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        status: "frozen",
        superseded: { by_baseline_id: "baseline-v1.2.0" },
      })
    )
    writeFileSync(
...
describe("scaffold + fixture gitignore the COMPLETE never-commit set, and ONLY that", () => {
  ...
  it("every rehearsal fixture .gitignore lists the complete never-commit set and none of the outputs that must stay tracked", () => {
    assert.ok(
      fixtureRoots.includes("pocket-ledger") && fixtureRoots.length > 1,
      `expected the rehearsal fixtures to be discovered, found ${fixtureRoots.join(", ")}`
    )
  ...
  it("the scaffold gitignore() template emits the complete never-commit set and none of the outputs that must stay tracked", () => {
    ...
    for (const line of NEVER_RE_INCLUDED)
      assert.ok(!lines.has(line), `scaffold must NOT re-include ${line} — the greenfield placeholder is delivered tracked, not re-included`)
