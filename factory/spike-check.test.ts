...
function spike({
  ...
  gate,
  sections,
}: { slug?: string; status?: string; gate?: string; sections?: SpikeSections } = {}): string {
...
const spk = (slug: string, opts: Omit<TraceOpts, "slug"> = {}) => spike({ slug, sections: { traceability: trace({ slug, ...opts }) } })
