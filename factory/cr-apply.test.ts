...
function seedSpike(filename: string, { status = "failed", reqId = "REQ-ARCH-001" }: { status?: string; reqId?: string } = {}) {
  ...
  write(
    `.vivicy/development/spikes/${filename}`,
    [
      `# S - ${slug}`,
      "",
      "## Traceability",
...
function fakeFreeze({ onFreeze }: { onFreeze?: () => void } = {}) {
  ...
  const runFreeze: NonNullable<ApplyChangeRequestArgs["runFreeze"]> = async ({
    repoRoot,
    version,
    previousVersion,
    approvedBy,
    ...
    write(
      `.vivicy/baselines/${baselineId}.json`,
      JSON.stringify(
        {
          schema_version: 1,
...
function reportSink() {
  ...
  return {
    recordReport: (r: Record<string, unknown>) => reports.push(structuredClone(r)),
    reports,
...
describe("applyChangeRequest — happy path (green)", () => {
  it("folds the canonical, freezes a patch bump, stamps docs_applied, spawns extraction, records phases", async () => {
    ...
    const result = await applyChangeRequest({
      ...
      runFreeze,
      runExtraction,
      recordReport: sink.recordReport,
  ...
  it("commits the applied canonical edit BEFORE freezing (else doc-baseline refuses a dirty tree)", async () => {
    ...
    const runFreeze: NonNullable<ApplyChangeRequestArgs["runFreeze"]> = async ({ version }) => {
      ...
      return {
        manifestPath: `.vivicy/baselines/${baselineId}.json`,
        baselineId,
        version,
        documentSetHash: `doc-${version}`,
...
describe("applyChangeRequest — retires the disproven spikes the CR folds (failed -> deferred)", () => {
  ...
  it("leaves a spike alone when the CR names its gate but the spike is NOT failed (no verified/pending downgrade)", async () => {
    ...
    assert.equal(result.status, "green")
    assert.equal(
      readSpikes(temp).find((s) => s.gate_id === verified.gate_id)?.status,
      "verified",
      "a verified spike named on the CR is not downgraded"
    )
...
describe("applyChangeRequest — a red reference-check blocks honestly", () => {
  ...
  it("names several broken links in the plural, subject and verb together", async () => {
    ...
    await applyChangeRequest({
      ...
      id: "CR-0001",
      spawnApplier,
      runFreeze: fakeFreeze().runFreeze,
      runExtraction: fakeExtraction().runExtraction,
      recordReport: reportSink().recordReport,
    })
...
describe("applyChangeRequest — refuses a CR that is not approved into the build", () => {
  it("blocks on an idea CR without touching it", async () => {
    ...
    seedCanonical()
    write(
      ".vivicy/change-requests/CR-0001-not-approved.md",
      [
        "---",
        "id: CR-0001",
