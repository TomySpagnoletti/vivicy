...
const ISSUE: AgentIssue = { id: "ISSUE-0001", transcript_dir: "ISSUES/ISSUE-0001", issue_path: "issues/ISSUE-0001.md", graph_refs: ["node:alpha", "node:beta"] };
...
test("legDepsForTarget composes the role prompt and appends the run context AFTER it", () => {
  ...
  assert.ok(prompt.indexOf("Fix ISSUE-0001") < prompt.indexOf("## Context for this run"), "the role prompt leads and the run context follows — a swap would put the run's instructions before the role that must obey them");
});
...
test("legDepsForTarget carries the REAL composePrompt, so a leg gets no unfilled slot and no per-stage wiring", () => {
  ...
  assert.doesNotMatch(prompt, /\{\{/, "every slot composePrompt owns is filled — a stub composer would leak the literal placeholder to the leg");
  ...
  assert.equal(deps.composePrompt("Role prompt.", ISSUE), "Role prompt.", "an empty context appends NOTHING — no separator invented on the stage's behalf");
});
...
test("the run context is appended AFTER composition, so slot-shaped text a stage splices in stays literal", () => {
  ...
  assert.match(prompt, /replace \{\{issue_id\}\} in the template/, "composing the CONCATENATION instead would substitute inside agent-written context (CR bodies, prior-attempt feedback) — that text is quoted evidence, never a template");
});

test("legDepsForTarget roots the leg at the target repo: abs() resolves against it, execRoot is it, and cwdFilter stays null", () => {
  const deps = legDepsForTarget(ROOT, "");

  assert.equal(deps.abs(".vivicy/development/reports/extraction-status.json"), resolve(ROOT, ".vivicy/development/reports/extraction-status.json"));
  ...
  assert.equal(deps.cwdFilter, null, "null asserts no two legs bound here are ever in flight in the same tree; dev-loop's own legDeps sets a real filter because its legs run N-way in sibling worktrees");
});
...
test("legDepsForVerbatimPrompt sends the pre-composed prompt untouched, ignoring both the role template and the issue", () => {
  ...
  assert.equal(deps.composePrompt("# vivi\n\nWhatever the role file says {{issue_id}}.", ISSUE), promptText, "no double-composition and no tidying: what lib/vivi.ts wrote, to the byte, is what the leg runs");
...
test("both bindings share ONE rooting — only the prompt differs", () => {
  ...
  assert.deepEqual(Object.keys(verbatim).sort(), fields, "a field set on one builder and not the other is the fork this seam exists to prevent");
  for (const key of fields) {
    if (key === "composePrompt") continue;
    assert.deepEqual(read(verbatim, key), read(composed, key), `${key} forked between the two bindings — every field but composePrompt comes from the one literal`);
  }
