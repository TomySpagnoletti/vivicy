...
test("impactedIssues returns issues referencing a changed or removed requirement", () => {
  ...
  const index = {
    issues: [
      { id: "ISSUE-0001", requirement_ids: ["REQ-A-001"] },
      { id: "ISSUE-0002", requirement_ids: ["REQ-A-002"] },
      { id: "ISSUE-0003", requirement_ids: ["REQ-A-009", "REQ-A-001"] },
    ],
  };
  assert.deepEqual(impactedIssues(drift, index), ["ISSUE-0002", "ISSUE-0003"]);
});
...
test("runReopen reopens exactly the impacted done issues and leaves unchanged ones done", () => {
  ...
  try {
    ...
    write(
      ...
      JSON.stringify({
        issues: [
          { id: "ISSUE-0001", requirement_ids: ["REQ-A-001"], graph_refs: ["node:a"] },
          { id: "ISSUE-0002", requirement_ids: ["REQ-A-002"], graph_refs: ["node:b"] },
        ],
      }),
    );
    write(".vivicy/development/issues/done/ISSUE-0001.md", "# done unchanged");
    write(".vivicy/development/issues/done/ISSUE-0002.md", "# done changed");
    ...
    assert.deepEqual(res.reopened, ["ISSUE-0002"]);
    assert.ok(existsSync(resolve(root, ".vivicy/development/issues/ISSUE-0002.md")), "reopened issue is active again");
    assert.ok(!existsSync(resolve(root, ".vivicy/development/issues/done/ISSUE-0002.md")), "reopened issue left done/");
    assert.ok(existsSync(resolve(root, ".vivicy/development/issues/done/ISSUE-0001.md")), "unchanged issue stays done");
    assert.equal(events.length, 1);
    assert.equal(events[0].event_type, "issue_reopened");
    assert.equal(events[0].issue_id, "ISSUE-0002");
