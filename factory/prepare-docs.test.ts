      ...
      JSON.stringify(
        {
          ...
          language,
          cycle: opts.cycle ?? { binding: "active", id: "project" },
          files: manifestFiles,
        },
        null,
        2
...
function openCycle(root: string, id: string): void {
  ...
  writeFileSync(
    abs,
    JSON.stringify({ status: "drafting", kind: "feature", id, opened_at: new Date().toISOString(), opened_by: "test" }, null, 2)
  )
}
...
test("completeBatches returns only manifest-carrying batches, id-sorted, skipping interrupted ones", () => {
  ...
  try {
    ...
    assert.deepEqual(
      completeBatches(root).map((b) => b.batchId),
...
test("a seed batch is ignored while the canonical is frozen and becomes unconsumed-active when its cycle opens", () => {
  ...
  try {
    ...
    assert.deepEqual(
      unconsumedActiveCycleBatches(root, null).map((b) => b.batchId),
      []
    )
...
test("crash-safe consumption: a failed batch is NOT marked consumed; a re-run retries only it", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      unconsumedActiveCycleBatches(root, failed).map((b) => b.batchId),
      ["2026-02-02"]
    )
...
test("a scratch a leg child still holds never fails the stage: the placement stands and the leftover is announced, never raised", async () => {
  ...
  try {
    ...
    assert.ok(
      existsSync(join(held, "flush.tmp")),
      "the held scratch really did survive the clear, so the run was proven against a real leftover"
    )
  } finally {
...
test("a green doc-prep that rejected a document emits a WARNING notification naming the drop", async () => {
  ...
  try {
    ...
    const report = await prepareDocs({ repoRoot: root, spawnLeg: NEVER_SPAWN })
    assert.equal(report.phase, "green")
    const lines = readFileSync(join(root, "notifications.jsonl"), "utf8")
      .trim()
      .split("\n")
      .map((l) => JSON.parse(l))
...
test("secret gate: a canonical doc carrying a high-confidence key is kept OUT of the canonical (rejected secret_detected), never placed", async () => {
  ...
  try {
    ...
    assert.ok(
      report.placed.some((p) => p.target === "canonical/ok.md"),
      "the clean sibling is still placed"
    )
...
test("secret gate: a generic high-entropy value is placed with a loud warning (a spec string is not a hard block)", async () => {
  ...
  try {
    ...
    assert.equal(report.phase, "green")
    assert.ok(
      report.placed.some((p) => p.target === "canonical/spec.md"),
      "placed despite the generic finding"
    )
...
test("the doc-prep summaries agree in number with the documents and batches they counted", async () => {
  ...
  try {
    ...
    assert.match(
      report.summary,
      /^doc-prep green for cycle project: 1 canonical document placed, 0 rejected, across 1 batch \(language eng\)/
    )
  } finally {
  ...
  try {
    ...
    assert.match(
      report.summary,
      /^doc-prep green for cycle project: 3 canonical documents placed, 0 rejected, across 2 batches \(language eng\)/
    )
  } finally {
