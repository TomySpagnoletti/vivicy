...
export function resolveFreezeVersion(repoRoot: string): string {
  ...
  for (const entry of readdirSync(dir)) {
    ...
    if (
      !best ||
      parsed[0] > best[0] ||
      (parsed[0] === best[0] && (parsed[1] > best[1] || (parsed[1] === best[1] && parsed[2] > best[2])))
    ) {
      best = parsed
    }
  }
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    const fixContext = isFix
      ? [
          ...
          .join("\n\n")
      : null
    const leg = await spawnExtractor({
      repoRoot,
      manifestPath,
      baselineId,
      cfg,
      attempt,
      checkOutput: fixContext,
      isFix,
      spikeMode,
      mapMode,
    })
...
function extractionIssue(): ExtractionIssue {
  return {
    id: TRANSCRIPT_DIRS.extraction,
    transcript_dir: TRANSCRIPT_DIRS.extraction,
    graph_refs: ["node:extraction"],
    path: ISSUE_INDEX_REL,
  }
}

function makeDefaultSpawnExtractor(
  options: ExtractIssuesOptions,
  baseCfg: Record<string, unknown>,
  legs: ResolvedLegs
): (args: SpawnExtractorArgs) => Promise<LegResult> {
...
function makeDefaultSpawnVerifier(
  ...
  baseCfg: Record<string, unknown>,
  legs: ResolvedLegs
): (args: SpawnVerifierArgs) => Promise<LegResult> {
  const promptsDir = options.promptsDir ?? FACTORY_PROMPTS_DIR
  const reviewer = legs?.reviewer ?? {
    actor: "codex",
    provider: "codex",
    model: CLI_DEFAULTS.codex.model,
    effort: CLI_DEFAULTS.codex.effort,
    fast: false,
  }
...
function makeDefaultSpawnLens(
  options: ExtractIssuesOptions,
  baseCfg: Record<string, unknown>,
  legs: ResolvedLegs
): (args: SpawnLensArgs) => Promise<LegRunResult> {
  const promptsDir = options.promptsDir ?? FACTORY_PROMPTS_DIR
  const reviewer = legs?.reviewer ?? {
    actor: "codex",
    provider: "codex",
...
function makeDefaultMapReview(
  options: ExtractIssuesOptions,
  cfg: Record<string, unknown>,
  legs: ResolvedLegs
): (args: MapReviewArgs) => Promise<MapReviewAggregate> {
...
function defaultVerifyFrozenManifest({
  repoRoot,
  manifestPath,
  baselineId,
}: {
  ...
  try {
    ...
    return {
      ...
      problems: [
        {
          issue: "*",
          kind: "unparseable_verdict",
          detail: `verdict JSON is invalid: ${error instanceof Error ? error.message : String(error)}`,
        },
      ],
    }
  }
...
function maybeRunDocPrep(repoRoot: string): void {
  ...
  if ((prep.status ?? 1) !== 0) {
    process.stdout.write(
      "extract-issues: document preparation did not go green (non-fatal, retryable); extraction proceeds on the canonical as-is\n"
    )
  }
}
...
if (cliEntry === fileURLToPath(import.meta.url)) {
  ...
  if (!repoRoot) {
    console.error("error: no target project configured. Set VIVICY_TARGET_ROOT to the absolute path of the project to extract.")
    process.exit(2)
  }
  maybeRunDocPrep(repoRoot)
