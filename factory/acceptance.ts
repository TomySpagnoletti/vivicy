...
function makeDefaultSpawnAcceptanceLeg(options: RunAcceptanceOptions): SpawnAcceptanceLeg {
  ...
  return async ({ repoRoot, manifestPath, baselineId, verdictRel }) => {
    ...
    const issue: AgentIssue = {
      id: TRANSCRIPT_DIRS.acceptance,
      transcript_dir: TRANSCRIPT_DIRS.acceptance,
      graph_refs: ["node:acceptance"],
      path: verdictRel,
    }
...
export async function runAcceptance(options: RunAcceptanceOptions = {}): Promise<AcceptanceReport> {
  ...
  if (
    !options.force &&
    prior &&
    (prior.phase === "green" || prior.phase === "findings") &&
    prior.baseline_id === baseline.baselineId &&
    prior.done_set_hash === hash
  ) {
    return prior
  }
  ...
  try {
    legResult = await spawnLeg({
      ...
      verdictRel: ACCEPTANCE_VERDICT_REL,
    })
  } catch (error) {
    report.phase = "failed"
  ...
  if (findings.length === 0) {
    ...
    report.phase = "failed"
    report.summary =
      "acceptance leg reported the product not accepted but listed no actionable findings — the verdict is unusable; retry the dev stage."
    return emit()
  }
