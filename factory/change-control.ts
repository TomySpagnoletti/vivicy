...
export function createChangeRequest({
  ...
  source = "agent",
  sourceEvidence = [],
  affectedVerificationGates = [],
  body = null,
  now,
}: CreateChangeRequestOptions = {}): { id: string; path: string } {
...
export function decideChangeRequest({ repoRoot, id, decision, decidedBy, evidenceRef, now }: DecideChangeRequestOptions = {}): {
  id: string
  path: string
  status: "accepted_current_build" | "rejected"
} {
  ...
  if (decision === "rejected") {
    ...
    if (!frozen) {
      throw new Error(
        `decideChangeRequest: cannot approve ${id} — no frozen baseline manifest under ${BASELINES_DIR}/ to record as previous_baseline_*`
      )
    }
...
export function stampChangeRequestApplied({ repoRoot, id, resulting, now }: StampChangeRequestAppliedOptions = {}): {
  id: string
  path: string
  status: "docs_applied"
} {
  ...
  for (const { file, fileNumber, fm } of crs) {
    ...
    if (!fm) {
      fail(
        ...
        "every CR opens with a --- frontmatter block",
        `add the CR frontmatter block to ${label}`
      )
      continue
    }
    ...
    if (!CR_ID.test(id)) {
      ...
      if (fileNumber === null || `CR-${String(fileNumber).padStart(4, "0")}` !== id) {
        fail(
          "cr_id_filename_match",
          label,
          `filename number ${fileNumber} vs id ${id}`,
          "the filename CR-####-slug.md number equals the frontmatter id",
          `rename ${label} or align its id`
        )
      }
    }
    ...
    if (!(CR_CLASSIFICATIONS as readonly string[]).includes(classification)) {
      fail(
        "cr_classification_enum",
        label,
        `classification="${classification}"`,
        `classification is one of ${CR_CLASSIFICATIONS.join(" | ")}`,
        `set a valid classification in ${label}`
      )
    }
    ...
    if (DECIDED_STATUSES.has(status)) {
      ...
      if (missing.length > 0) {
        fail(
          "cr_decision_evidence",
          label,
          `decided status "${status}" but missing: ${missing.join(", ")}`,
          "a decided CR records owner_decision_by, owner_decision_at, and owner_decision_evidence",
          `populate the decision-evidence fields in ${label}`
        )
      }
    }
    ...
    if (PREVIOUS_REQUIRED.has(status)) {
      ...
      if (missing.length > 0) {
        fail(
          "cr_previous_baseline",
          label,
          `status "${status}" but missing previous_baseline field(s): ${missing.join(", ")}`,
          "accepted_current_build onward records the pre-change baseline identity",
          `record the previous_baseline_* fields in ${label}`
        )
      }
    }
    if (RESULTING_REQUIRED.has(status)) {
      ...
      if (missing.length > 0) {
        fail(
          "cr_resulting_baseline",
          label,
          `status "${status}" but missing resulting_* field(s): ${missing.join(", ")}`,
          "docs_applied onward records the resulting baseline identity",
          `record the resulting_* fields in ${label}`
        )
      } else if (!manifestHashes.has(String(fm.resulting_manifest_hash))) {
        fail(
          "cr_resulting_manifest_exists",
          label,
          `resulting_manifest_hash "${fm.resulting_manifest_hash}" matches no manifest in ${BASELINES_DIR}/`,
          "the resulting baseline manifest exists and was generated by the freeze",
          `generate/verify the resulting baseline before marking ${label} docs_applied`
        )
      }
    }
  }
  ...
  for (let i = 0; i < sorted.length; i += 1) {
    if (sorted[i] !== i + 1) {
      fail(
        "cr_sequential",
        CHANGE_REQUESTS_DIR,
        `CR numbers ${sorted.join(", ")} are not a gap-free 1..N sequence`,
        "CR ids are sequential CR-0001..CR-####N with no gaps or duplicates",
        "renumber the CRs so their ids form a contiguous sequence"
      )
      break
    }
  }
...
function validateSupersedesGraph(crs: ChangeRequestRecord[], fail: FailFn): void {
  ...
  for (const cr of crs) {
    ...
    for (const target of toList(fm.supersedes)) {
      ...
      if (!other) {
        fail(
          "cr_supersedes_target",
          label,
          `supersedes "${target}" which does not exist`,
          "every supersedes target is a real CR",
          `fix the supersedes list in ${label}`
        )
      } else if (String(other.superseded_by ?? "") !== String(fm.id)) {
        fail(
          "cr_supersedes_consistency",
          label,
          `${fm.id} supersedes ${target}, but ${target}.superseded_by="${other.superseded_by ?? "null"}"`,
          "a supersedes edge is mirrored by the target's superseded_by",
          `set superseded_by: ${fm.id} in ${target}`
        )
      }
    }
  }
...
function validateNoCrOnlyRequirements(root: string, fail: FailFn): void {
  ...
  for (const req of requirements) {
    ...
    if (allFromCr) {
      fail(
        "requirement_sourced_only_from_cr",
        `${CATALOG_PATH} ${req.id}`,
        `${req.id} sources only from change-requests/`,
        "an active requirement is sourced from the canonical baseline, never only from a CR",
        `cite ${req.id} to canonical lines (apply the CR to canonical and re-extract), or remove it`
      )
    }
  }
}
...
function renderNewChangeRequest({
  id,
  title,
  classification,
  source,
...
function done(errors: string[], scope = ""): ChangeControlCheckResult {
  return {
    ...
    placeholder: false,
    summary:
      errors.length > 0 ? `change-control: FAILED with ${errors.length} error(s)` : `change-control: OK${scope ? ` (${scope})` : ""}`,
  }
}
