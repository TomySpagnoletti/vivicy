...
test("buildClaudeArgs isolates the leg from the machine's Claude Code layers while keeping the prompt, the session id and the model args", () => {
  const args = buildClaudeArgs({ prompt: "do the thing", uuid: "0f9e8d7c-6b5a-4321-9876-543210fedcba", modelArgs: ["--model", "claude-opus-4-8", "--effort", "xhigh"] });
  ...
  assert.deepEqual(CLAUDE_ISOLATION_ARGS, ["--safe-mode"], "one flag disables the whole user layer (CLAUDE.md, skills, plugins, hooks, MCP servers, custom agents) and leaves auth resolution alone");
});
...
test("buildCodexArgs isolates the leg from the machine's Codex layers while keeping the prompt, the exec root and the model args", () => {
  const args = buildCodexArgs({ prompt: "do the thing", root: "/tmp/target", modelArgs: ["-m", "gpt-5.5", "-c", 'model_reasoning_effort="high"'] });
  assert.deepEqual(args, [
    "exec",
    "do the thing",
    "--ignore-user-config",
...
test("isolateAgentEnv drops every agent-CLI-configuring variable and keeps exactly the auth carve-out", () => {
  ...
  const neutral = { PATH: "/usr/bin", HOME: "/home/dev", LANG: "en_US.UTF-8", AWS_PROFILE: "bedrock", GOOGLE_APPLICATION_CREDENTIALS: "/gcp.json", CLOUD_ML_REGION: "europe-west1", VIVICY_TARGET_ROOT: "/repo", VIVICY_CLAUDE_MODEL: "claude-opus-4-8" };
...
test("every authentication family is carried whole — a kept credential is never left unreachable", () => {
  ...
  assert.ok(AGENT_ENV_AUTH_PASSTHROUGH.has("CLAUDE_CODE_USE_GATEWAY"), "the gateway is the one provider with a selector and no skip-auth flag; it must still ride through");
  assert.deepEqual(
    [...AGENT_ENV_AUTH_PASSTHROUGH].sort(),
...
test("agentEnv applies the isolation to the orchestrator's own environment", () => {
  const injected = { CLAUDE_CODE_PROBE_MARKER: "1", ANTHROPIC_MODEL: "claude-haiku-4-5", VIVICY_PROBE_MARKER: "1", ANTHROPIC_API_KEY: "sk-ant-api03-probe" };
...
test("transcript capture follows the same credential-store overrides the isolation lets through", () => {
  ...
  try {
    ...
    assert.equal(withEnv({ CLAUDE_CONFIG_DIR: claudeDir }, () => captureClaudeTranscript(uuid, dest)), true);
    assert.equal(readFileSync(dest, "utf8"), '{"type":"user"}\n');
    assert.equal(withEnv({ CLAUDE_CONFIG_DIR: join(root, "empty") }, () => captureClaudeTranscript(uuid, dest)), false, "a redirected config dir with no session for the uuid is a miss, never a silent read of the default dir");
    ...
    assert.equal(withEnv({ CODEX_HOME: codexHome }, () => findNewestCodexRollout(1_999_999_999_000, null)), rollout);
    assert.equal(withEnv({ CODEX_HOME: join(root, "empty") }, () => findNewestCodexRollout(1_999_999_999_000, null)), null);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test("the spawn seam hands the real CLI the isolation flags and an isolated environment, for both providers", () => {
  const claude = fakeCliRun("claude", ({ cfg, issue, deps }) => runClaudeLeg({ actor: "claude", role: "implementer", provider: "claude" }, issue, cfg, deps));
  ...
  const codex = fakeCliRun("codex", ({ cfg, issue, deps }) => runCodexLeg({ actor: "codex", role: "implementer", provider: "codex" }, issue, cfg, deps));
  ...
  for (const [provider, run] of [["claude", claude], ["codex", codex]] as const) {
    assert.equal(run.env.CLAUDE_CODE_PROBE_MARKER, undefined, `${provider}: a machine-level CLAUDE_CODE_* variable must not survive the seam`);
