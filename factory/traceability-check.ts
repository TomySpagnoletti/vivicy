...
export function runTraceabilityCheck(options: { repoRoot?: string } = {}): TraceabilityCheckResult {
  ...
  if (!existsSync(resolveInside(root, CATALOG))) {
    fail(
      "catalog_required",
      CATALOG,
      "file missing while issues exist",
      "the Requirement Catalog exists once extraction has run",
      `author ${CATALOG}`
    )
    return done(errors)
  }
  if (!existsSync(resolveInside(root, MATRIX))) {
    fail(
      ...
      MATRIX,
      "file missing while issues exist",
      "the Traceability Matrix exists once extraction has run",
      `author ${MATRIX}`
    )
    return done(errors)
  }
  ...
  try {
    ...
    fail(
      "catalog_parse",
      CATALOG,
      `unable to parse: ${(error as Error).message}`,
      "the Requirement Catalog is valid JSON",
      `fix ${CATALOG}`
    )
    return done(errors)
  }
