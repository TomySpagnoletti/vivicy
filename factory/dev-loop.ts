...
// the extractor in extract-issues.ts). dev-loop binds them to its own root
...
// An issue as it appears in the issue index / on disk. Every field beyond `id` is
// optional because the loop tolerates partial issues (tests pass minimal ones) and
// reads each field defensively (Array.isArray guards throughout).
...
// The minimal issue fields each pure scheduler helper actually reads. Declaring the
// narrow shape (rather than the whole Issue) keeps the helpers precise AND lets the
// unit tests exercise them with partial fixtures; a full Issue satisfies each.
...
// The canonical leg-result shape the pipeline understands (from agent-spawn.ts),
// augmented with the quota-retry outcome by runLegWithQuota.
...
// What a leg step returns: a real leg result, or nothing (a test fake may just
// perform a side effect and return void). The loop reads the result defensively
// (optional chaining), so a void return is tolerated exactly as in the JS original.
type LegStepReturn = LegResult | void;

// One quota window record (5h / weekly): a real used percentage or an honest null,
// the derived remaining, and an ISO reset time (or null when unknown).
...
// The gate-run verdict returned by the gate runners + fake gates.
...
// A snapshot of a gate-run evidence file: its path plus the parsed record (or null
// when unreadable), embedded verbatim in a post-merge integration block.
interface GateEvidenceSnapshot {
  path: string;
  record: unknown;
}

// A readiness verdict file's parsed shape (S8/G5). `verdict` is validated against
// READINESS_VERDICTS before use; the optional fields are read defensively.
...
// The outcome of a readiness route: proceed, or park with a reason.
type ReadinessOutcome =
  | { status: "implementable" }
  | { status: "parked"; reason: string; parkedRel?: string };

// A readiness leg step: the injected fake (a test) or the bound default runner.
// It may be sync or async; the sync scheduler only ever hands it a sync one.
type ReadinessRunner = (issue: Issue, cfg: Config) => LegStepReturn | Promise<LegStepReturn>;

// The resolved steps a SYNC issue cycle drives (defaults already filled by the loop).
...
// The resolved steps an ASYNC (parallel) issue cycle drives; legs + gate may be async.
...
// The result of running one issue through implement -> review -> gate.
...
// One processed-issue entry the loop returns per issue it settled.
...
// The notification level, mirroring notify.ts's local NotificationLevel.
type NotifyLevel = "info" | "success" | "warning" | "error";

// A progress event as the loop assembles it before emit() stamps the session_ref
// and hands it to recordProgressEvent (which re-validates it as a ProgressEvent).
// graph_refs is optional here because it flows from an Issue (permissive), though
// every real emit supplies it; the ledger writer enforces non-empty at runtime.
interface EmitEvent {
  event_type: string;
  issue_id: string;
  graph_refs?: string[];
...
// The result of integrating a worktree branch onto the integration branch.
...
// One graph item's recorded state in the progress ledger (only the fields the loop
// reads back are typed; the writer, progress-ledger.ts, owns the full shape).
...
// The progress ledger, as far as the loop reads it back.
interface Ledger {
...
// A read-only architecture index: node id -> cluster, node id -> adjacent node ids.
export interface ArchitectureIndex {
  clusterByNode: Map<string, string>;
  adjacencyByNode: Map<string, Set<string>>;
}

// One agent's persisted quota state. Advisory (never load-bearing); the writer
// merges partial updates, so every field is optional.
...
// The durable quota-state file shape.
interface QuotaState {
  updated_at: string | null;
  agents: Record<string, AgentQuotaState>;
}

// A `<id>-parked.json` report: the issue's identity stamped at park time so the
// park clears the instant the issue file changes.
...
// The current identity of an issue file (content hash + mtime + its relative path).
...
// The root-resolution deps dev-loop binds onto the shared agent-spawn leg runners.
interface LegDeps {
...
// The four disjoint token sets that make up an issue's architecture footprint.
...
// The injectable steps a caller (or a test) can override on runLoop / runLoopParallel.
// Every entry is optional; the loop supplies a real default for each unset one.
...
// The loop configuration: DEFAULT_CONFIG plus the injectable test seams. Every
// field is optional because a caller may pass any subset (merged over DEFAULT_CONFIG).
...
export const DEFAULT_CONFIG: Config = {
  ...
  //   See factory/project-config.ts (resolveGateCommand) — the single owner of
...
export function parseCodexQuotaWindows(rolloutText: string | null | undefined): QuotaWindows {
  ...
  for (const line of text.split("\n")) {
    ...
    if (rl && typeof rl === "object") limits = rl as { primary?: unknown; secondary?: unknown }; // keep the last seen
  }
  if (!limits) return {};
...
export function parseClaudeQuotaWindows(transcriptText: string | null | undefined): QuotaWindows {
  ...
  for (const line of text.split("\n")) {
    ...
    if (obj?.type === "rate_limit_event" && obj.rate_limit_info) {
      info = obj.rate_limit_info as { rateLimitType?: unknown; resetsAt?: unknown }; // last wins
    }
  }
  if (!info) return {};
  const windows: QuotaWindows = {};
...
// extractor (extract-issues.ts) so the model/effort flags, max-permission flags,
// actor/role env, and transcript capture are defined ONCE in
// agent-spawn.ts and never diverge between the two drivers. dev-loop.ts binds
...
export function defaultRunGate(issue: Issue, cfg: Config): GateResult {
  ...
  // with NO hidden Node assumption (project-config.ts owns the resolution).
...
export async function defaultRunGateAsync(issue: Issue, cfg: Config): Promise<GateResult> {
  const execRoot = execRootOf(cfg);
  // Same polyglot resolution as the sync gate (project-config.ts is the one owner).
  const gateCommand = resolveGateCommand({
    issue,
    targetRoot: execRoot,
    explicitDefault: cfg.defaultGateCommand,
...
// generated ONCE at extraction (extract-issues.ts) and committed there. The
...
// Inlined here (not imported from extract-issues.ts) to avoid an import cycle.
...
function emit(cfg: Config, event: EmitEvent): ReturnType<typeof recordProgressEvent> {
  ...
  // recordProgressEvent re-validates the event (graph_refs non-empty, known type)
  // at runtime; the cast crosses from the loop's permissive Issue-derived shape to
  // the validated ProgressEvent contract, adding the mechanically-derived session_ref.
...
export function runLegWithQuota(
  ...
  for (let attempt = 1; ; attempt += 1) {
    // A void return (a side-effect-only fake) reads back as undefined here.
    const legResult = runLeg(issue, cfg) as LegResult | undefined;
    // combinedOutput (agent-spawn.ts) types its param as leg-timeout's LegResult,
    // whose stdout/stderr are required; a leg's `.result` here is the same runtime
    // spawnSync shape but our permissive LegProcessResult marks them optional, so we
    // bridge that cross-cluster optional/required gap at the call.
...
function runReadinessSync(issue: Issue, cfg: Config, runReadinessStep: ReadinessRunner): ReadinessOutcome {
  ...
  for (let attempt = 1; attempt <= 2 && !verdict; attempt += 1) {
    // The sync scheduler only ever hands runReadinessSync a synchronous step (the
    // resolver's sync branch, or a sync fake), so it is safe to drive it through the
    // sync quota runner.
...
// watchdog (set by leg-timeout.ts). Used so a block caused by a wedged CLI says
...
export function runLoop(
  ...
  // The sync path's injected steps are synchronous (its tests supply sync fakes);
  // LoopSteps allows async to also serve the parallel loop, so the leg/gate steps
  // are cast to the sync CycleSteps shape here.
  const resolvedSteps = {
...
export async function runLoopParallel(userConfig: Partial<Config> = {}, steps: LoopSteps = {}): Promise<ProcessedIssue[]> {
  ...
  const processed: ProcessedIssue[] = [];
  const running = new Map<string, Promise<ProcessedIssue>>(); // issue.id -> Promise of its settled result
  const runningIssueById = new Map<string, Issue>(); // issue.id -> issue (for independence checks)
  const blocked = new Set<string>(); // issue ids that settled blocked (and never run again)
      ...
      return await withIntegrationLock(cfg, async (): Promise<ProcessedIssue> => {
        ...
        if (!merge.ok) {
          ...
          if (steps.runMergeResolver !== false) {
            ...
            // Guarded by `steps.runMergeResolver !== false`, so the step is the
            // resolver function (never false) here.
