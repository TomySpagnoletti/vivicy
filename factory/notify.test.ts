...
test("notify is a strict no-op without a runtime dir", () => {
  ...
  try {
    assert.equal(notify({ level: "warning", stage: "S9", event: "gate_failed", message: "x" }), false)
  } finally {
...
test("notify appends the contract shape with unique ids", () => {
  ...
  try {
    assert.equal(
      notify({ level: "warning", stage: "S9", event: "gate_failed", message: "ISSUE-0001: gate red" }, { runtimeDir: dir }),
      true
    )
    assert.equal(
      notify(
        { level: "error", stage: "S10", event: "post_merge_gate_failed", message: "ISSUE-0002: reverted" },
