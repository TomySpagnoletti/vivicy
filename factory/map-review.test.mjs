...
test("runMapReview fans out one leg per lens, aggregates findings, and marks actionable", async () => {
  const spawned = [];
  const spawnLens = async ({ lens }) => {
    spawned.push(lens.key);
    return { transcriptRel: `t-${lens.key}` };
  };
  const findingsByLens = { security: { findings: [{ target: "node:a", detail: "x", real: true }] } };
  const readFindings = ({ lensKey }) => findingsByLens[lensKey] ?? { findings: [] };
  const lenses = [
