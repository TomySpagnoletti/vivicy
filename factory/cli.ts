...
function passthrough(command: string, args: string[], { cwd, env }: { cwd?: string; env?: NodeJS.ProcessEnv } = {}): void {
  const child = spawn(command, args, {
    cwd,
    stdio: "inherit",
    env: { ...process.env, ...env },
...
function countPendingCrs(target: string): number {
  return readCrSummaries(target).filter((c) => c.status === "idea" || c.status === "under_review").length
}
...
function startSupervisor(argv: string[], opts: Opts, mode: string): void {
  ...
  if (isSpecCycleOpen(target)) {
    return fail(
      json,
      EXIT_REFUSAL,
      "a drafting spec cycle is open — run `vivicy extract` to freeze it (or `vivicy cycle cancel`) before building",
      {
        code: "cycle_state",
      }
    )
...
function cmdPrepareReport(argv: string[]): void {
  ...
  if (json) {
    ...
    note(
      `doc-prep: ${report.phase ?? "?"} (cycle ${report.cycle_id ?? "none"}, ${countOf(consumed, "batch", "batches")} consumed, ${pending} pending, language ${report.language ?? "?"}) — ${report.summary ?? ""}`
    )
...
async function cmdRetryStage(argv: string[], opts: Opts): Promise<void> {
  ...
  if (!action) {
    return fail(json, EXIT_USAGE, `stage "${stage ?? ""}" is not retryable`, {
      code: "unsupported_stage",
      supported: Object.keys(RETRYABLE_STAGES),
    })
  }
  ...
  if (action === "cancel") {
    ...
    if (manifest) {
      const verify = spawnSync(
        process.execPath,
        [scriptPath("doc-baseline.ts"), "verify", "--manifest", manifest, "--require-status", "frozen"],
        {
          cwd: target,
          encoding: "utf8",
          env: childEnv(target, opts),
      ...
      if ((verify.status ?? 1) !== 0) {
        return fail(
          json,
          EXIT_REFUSAL,
          "the canonical has drifted from the frozen baseline — extract to freeze the evolution, or revert the edits first",
          { code: "cycle_state" }
        )
      }
    }
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "loop": {
      const target = takeFlag(argv, "--target")
      const env = target ? { VIVICY_TARGET_ROOT: resolve(target), VIVICY_RUNTIME_DIR: projectDir(opts, resolve(target)) } : {}
