...
test("verify --require-min-version rejects a frozen 0.x as an extraction source (E4)", () => {
  ...
  try {
    ...
    const tooLow = runCli(root, ["verify", "--manifest", ".vivicy/baselines/baseline-v0.2.0-draft.json", "--require-min-version", "1.0.0"])
...
test("--product overrides the derived name", () => {
  ...
  try {
    writeDoc(root, "01-a.md", "# Doc One\n\nbody alpha\n")
    writePackage(root, "formula")
    assert.equal(runCli(root, ["generate", "--version", "1.0.0", "--status", "draft", "--product", "Custom Brand"]).status, 0)
...
test("freeze refuses when a high-confidence secret is in the corpus, names the file/line-class redacted, and writes no manifest", () => {
  ...
  try {
    ...
    assert.doesNotMatch(
      res.stderr,
      new RegExp(PLANTED_KEY.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")),
      "the refusal never re-prints the secret"
    )
