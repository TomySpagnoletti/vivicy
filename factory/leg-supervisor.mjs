...
//
// Contract (the parent in agent-spawn.mjs depends on it):
...
function killTree(child, graceMs, onKilled) {
...
function main() {
  ...
  try {
    ...
    finish({ status: null, signal: null, timedOut: false, timeoutReason: null, spawnError: String(error?.message ?? error) });
    return;
  }
  ...
  child.on("error", (error) => {
    finish({ status: null, signal: null, timedOut, timeoutReason, spawnError: String(error?.message ?? error) });
  });
