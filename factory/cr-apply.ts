...
export async function applyChangeRequest(args: ApplyChangeRequestArgs = {}): Promise<TerminalReport> {
  ...
  if (status !== "accepted_current_build") {
    return terminal(recordReport, "blocked", "resolve", id, {
      summary: `cr-apply: CR ${id} is "${status}", the application chain only runs on accepted_current_build`,
    })
  }
  ...
  if (!/^\d+\.\d+\.\d+$/.test(previousVersion)) {
    return terminal(recordReport, "blocked", "freeze", id, {
      summary: `cr-apply: CR ${id} has no valid previous_baseline_version to bump from (got "${previousVersion}")`,
    })
  }
  ...
  if (!committed.committed) {
    return terminal(recordReport, "blocked", "commit", id, {
      summary: `cr-apply: could not commit the applied canonical edit for ${id} before freezing (git add/commit failed)`,
    })
  }
  ...
  try {
    ...
    return terminal(recordReport, "blocked", "freeze", id, {
      summary: `cr-apply: freeze failed for ${id}: ${error instanceof Error ? error.message : String(error)}`,
    })
  }
  ...
  try {
    ...
    return terminal(recordReport, "blocked", "stamp", id, {
      baseline,
      summary: `cr-apply: could not stamp ${id} docs_applied: ${error instanceof Error ? error.message : String(error)}`,
    })
  }
  ...
  if (retired.length > 0) {
    ...
    if (!committedRetire.committed) {
      return terminal(recordReport, "blocked", "retire_spikes", id, {
        baseline,
        retired,
        summary: `cr-apply: could not commit the retired ${countForm(retired.length, "spike", "spikes")} ${retired.join(", ")} for ${id} before re-extraction (git add/commit failed)`,
      })
    }
  }
...
function terminal(
  ...
  phase: string,
  cr: string,
  extra: Omit<TerminalReport, "status" | "phase" | "cr">
): TerminalReport {
...
function defaultRecordReport(repoRoot: string, id: string, report: ReportSnapshot): void {
  ...
  writeFileSync(
    abs,
    `${JSON.stringify({ ...report, updated_at: (report as Record<string, unknown>).updated_at ?? new Date().toISOString() }, null, 2)}\n`
  )
