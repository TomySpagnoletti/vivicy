...
const repoRoot = targetOverride && targetOverride.trim().length > 0 ? resolve(targetOverride) : resolve(scriptDir, "../..")
...
const defaultInclude = [".vivicy/canonical/**/*.md"]
...
function verify(args: ParsedArgs): void {
  ...
  if (args["require-status"] === "frozen") {
    ...
    if (!isNonEmptyString(approval.approved_by) || !isNonEmptyString(approval.approved_at) || !isNonEmptyString(approval.approval_ref)) {
      fail(
