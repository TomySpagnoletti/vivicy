...
//
// It reuses the same leg infrastructure the extractor does (agent-spawn.mjs +
// dev-loop.mjs helpers), built minimally like extract-issues.mjs. Honest failure
...
//
// Usage: node verify-upload.mjs --staging <abs>
...
// a product issue (identical role to extract-issues.mjs's extractionIssue()).
 ...
 *   spawnVerifier({ stagingDir, normalizedDir, reportPath, targetRoot, cfg })
 *       -> { transcriptRel?, output?, result? }   (the agent leg; writes report.json)
 *   readReport({ reportPath })  -> { verdict, problems, summary } | null
 *   writeReport({ reportPath, report })            (persist a report the script authors)
 *
 * @returns {{ verdict: "green"|"red", report, transcriptRel? }}
 */
export async function verifyUpload(options = {}) {
  ...
  // reports a timeout (leg-timeout.mjs sets result.timedOut) OR no report exists,
...
function redReport(summary, kind) {
...
function makeDefaultSpawnVerifier(options, baseCfg, legs) {
  ...
  const leg = { ...implementer, role: "upload-verifier" };
  return async ({ stagingDir, normalizedDir, reportPath, targetRoot, cfg }) => {
...
function verifierContext({ normalizedDir, reportPath, targetRoot }) {
...
// prompt context onto the role prompt (same wrapping extract-issues.mjs uses).
function legDepsForTarget(legCfg, issue, execRoot, context) {
...
function defaultReadReport({ reportPath }) {
...
function defaultWriteReport({ reportPath, report }) {
