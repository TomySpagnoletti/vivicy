...
test("acceptanceStageNeeded gates on applicability, settlement, baseline, and done-set", () => {
  ...
  assert.equal(acceptanceStageNeeded(baseline, { phase: "green", baseline_id: "baseline-v1.0.0", done_set_hash: "abc" }, at), false, "settled green");
  assert.equal(acceptanceStageNeeded(baseline, { phase: "findings", baseline_id: "baseline-v1.0.0", done_set_hash: "abc" }, at), false, "settled findings");
  assert.equal(acceptanceStageNeeded(baseline, { phase: "checking", baseline_id: "baseline-v1.0.0", done_set_hash: "abc" }, at), true, "still running is not settled");
  assert.equal(acceptanceStageNeeded(baseline, { phase: "failed", baseline_id: "baseline-v1.0.0", done_set_hash: "abc" }, at), true, "failed re-runs");
  assert.equal(acceptanceStageNeeded(baseline, { phase: "green", baseline_id: "baseline-v0.9.0", done_set_hash: "abc" }, at), true, "baseline changed");
  assert.equal(acceptanceStageNeeded(baseline, { phase: "green", baseline_id: "baseline-v1.0.0", done_set_hash: "zzz" }, at), true, "done-set changed");
});
...
test("a clean verdict flips the acceptance report green and drafts no change requests", async () => {
  ...
  try {
    const report = await runAcceptance({
      repoRoot: root,
      spawnLeg: verdictLeg({ accepted: true, scenarios: [{ id: "happy", verification: "executed", result: "pass" }, { id: "offline", verification: "read_only", result: "unverifiable_without_run_story" }], findings: [] }),
    });
...
test("an invalid proposed classification is coerced to minor_product_change (P5 — orchestrator enforces)", async () => {
  ...
  try {
    const report = await runAcceptance({
      repoRoot: root,
      spawnLeg: verdictLeg({ accepted: false, findings: [{ gap: "a real whole-product gap", title: "Fix the seam", classification: "totally-bogus" }] }),
    });
...
test("the leg writes its verdict to the reserved report path, not the committed report", async () => {
  ...
  try {
    ...
    assert.notEqual(ACCEPTANCE_VERDICT_REL, ACCEPTANCE_REPORT_REL, "the leg's proposal and the orchestrator's enforced report are distinct files");
  } finally {
    cleanup(root);
  }
});
