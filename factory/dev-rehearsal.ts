...
//
// Modes:
//   node factory/dev-rehearsal.ts            run the real two-agent loop
//   node factory/dev-rehearsal.ts --dry      validate the harness with fake agents
//   node factory/dev-rehearsal.ts --dry --concurrency=N
...
/** A processed issue as returned by the dev-loop (only id + status are read here). */
interface ProcessedIssue {
  id: string;
  status: string;
}

/** What the dry legs read off the issue the dev-loop hands them. */
interface LegIssue {
  id: string;
}

/** What the dry legs read off the per-issue cfg the dev-loop hands them. */
interface LegCfg {
  execRoot?: string;
}

/** Minimal shape of the static architecture map the rehearsal reads/projects. */
...
/** Minimal shape of the progress ledger the rehearsal reads. */
interface RehearsalLedger {
  graph_item_states?: { status?: string; transcript_refs?: unknown }[];
}

/** Minimal shape of the issue index the rehearsal reads. */
