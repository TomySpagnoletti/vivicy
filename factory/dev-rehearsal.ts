...
async function main(): Promise<void> {
  ...
  record(
    "generate-viewer-data (pre-loop)",
    r.status === 0 && (preData?.development?.issues?.length ?? 0) > 0,
  ...
  record(
    "extraction corpus committed (map tracked, clean tree)",
    corpusClean && mapCommittedPreLoop,
    mapCommittedPreLoop ? "map tracked" : "map NOT tracked"
  )
  ...
  const staticBakedVerified = (staticMapPreLoop.development?.graph_item_states ?? []).filter((s) => s.status === "verified").length
  record(
  ...
  if (concurrency > 1) {
    ...
    const worktreesLeft = existsSync(join(temp, ".vivicy-worktrees"))
      ...
      : 0
    record(
      `parallel (N=${concurrency}): every issue settled exactly once`,
      doneOnce && order.length > 0,
      `${order.length} settled: ${order.join(", ")}`
    )
  ...
  if (!existsSync(join(temp, ".vivicy/development/progress-ledger.json"))) {
    record(
      "temp workspace survived the run",
      false,
      "workspace vanished mid-run — re-run with no concurrent process touching the OS temp dir"
    )
  ...
  record(
    "ledger: graph items verified with transcript refs",
    verifiedStates.length > 0 && withTranscripts.length === verifiedStates.length,
    `${verifiedStates.length} verified, ${withTranscripts.length} with transcripts`
  )
...
async function projectLedgerOntoMap(temp: string, staticMap?: RehearsalMap | null, ledger?: unknown): Promise<RehearsalMap | null> {
  const map =
    staticMap ??
    (existsSync(join(temp, ".vivicy/architecture-map/architecture-data.json"))
...
async function runDocPrepScenario(): Promise<void> {
  ...
  try {
    ...
    const messyDoc =
      "Le produit permet à un utilisateur de gérer un catalogue d'articles avec recherche et pagination sur tout le jeu de données."
...
function writeUploadBatch(root: string, id: string, files: Record<string, string>, language: string, cycle: RehearsalCycle): void {
  ...
  writeFileSync(
    join(batchDir, "manifest.json"),
    JSON.stringify({ batchId: id, createdAt: new Date().toISOString(), language, cycle, files: manifestFiles }, null, 2)
  )
}
...
async function runCycleBatchScenarios(): Promise<void> {
  ...
  try {
    ...
    const bothPlaced =
      existsSync(join(progressive, ".vivicy/canonical/one.md")) && existsSync(join(progressive, ".vivicy/canonical/two.md"))
    record(
  ...
  try {
    ...
    writeSpecCycle(seed, {
      status: "drafting",
      kind: "feature",
      id: "cycle-2026-rehearsal-next",
      opened_at: new Date().toISOString(),
      opened_by: "owner:dev-rehearsal",
    })
...
function writeAcceptanceFixture(root: string): void {
  ...
  write(
    ...
    JSON.stringify({ schema_version: 1, baseline_id: "baseline-v1.0.0", version: "1.0.0", status: "frozen", files: [] }, null, 2)
  )
}
...
async function runAcceptanceScenarios(): Promise<void> {
  ...
  try {
    ...
    const report = await acc.runAcceptance({
      ...
      spawnLeg: async ({ repoRoot, verdictRel }: { repoRoot: string; verdictRel: string }) => {
        ...
        writeFileSync(
          ...
          JSON.stringify({
            accepted: true,
            scenarios: [{ id: "end-to-end", verification: "read_only", result: "unverifiable_without_run_story" }],
            findings: [],
          })
        )
      },
    ...
    const noCrs =
      !existsSync(join(clean, ".vivicy/change-requests")) ||
      readdirSync(join(clean, ".vivicy/change-requests")).filter((f) => f.endsWith(".md")).length === 0
    record(
  ...
  try {
            ...
            findings: [
              {
                ...
                gap: "the ISSUE-0001/ISSUE-0002 seam drops the tax line, so the checkout total is not tax-inclusive end to end",
    ...
    const crs = existsSync(join(defect, ".vivicy/change-requests"))
      ? readdirSync(join(defect, ".vivicy/change-requests")).filter((f) => /^CR-\d{4}-.*\.md$/.test(f))
      : []
...
function plantRecurringFailures(root: string): void {
  ...
  for (const iss of ["ISSUE-0001", "ISSUE-0002"]) {
    write(
      `.vivicy/development/gates/${iss}-gate.json`,
      JSON.stringify(
        {
          gate_id: `gate:test:${iss}`,
          issue_id: iss,
          command: "npm test",
          exit_code: 1,
          status: "fail",
    ...
    write(
      `.vivicy/development/reports/${iss}-blocked.json`,
      JSON.stringify({ kind: "quota", reason: "provider quota exhausted mid-issue", issue_id: iss }, null, 2)
    )
  }
}
...
async function runRetroScenarios(): Promise<void> {
  ...
  try {
        ...
        writeFileSync(
          ...
          JSON.stringify({
            recurring_classes: [
              {
                id: "gate-flake-test",
                kind: "gate_flake",
                signature: "the npm test gate failed on two issues",
                occurrences: 2,
                evidence: [".vivicy/development/gates/ISSUE-0001-gate.json", ".vivicy/development/gates/ISSUE-0002-gate.json"],
              },
              {
                id: "blocked-quota",
                kind: "blocked_cause",
                signature: "two issues blocked on provider quota exhaustion",
                occurrences: 2,
              ...
              { id: "one-off", kind: "review_finding", signature: "a single review finding", occurrences: 1 },
              {
                id: "fake-count",
                kind: "gate_flake",
                signature: "claims five recurrences but cites no witnesses",
                occurrences: 5,
                evidence: [],
              },
            ],
            proposals: [
              {
                landing: "settings",
                title: "Lower concurrency to smooth quota bursts",
                rationale: "two issues blocked on quota",
                detail: "Set maxParallel to 2 so the run does not exhaust the provider quota in bursts.",
                addresses: ["blocked-quota"],
              },
              {
                landing: "method_block",
                title: "Prime the gate before the first run",
                rationale: "the test gate flaked on two issues",
                detail:
                  "Add a method-block bullet: the stack-setup issue warms the toolchain before the first gate so a cold-start flake never blocks an issue.",
                addresses: ["gate-flake-test"],
              },
            ],
          })
        )
    ...
    const derivedOccurrences = (report.recurring_classes ?? []).every(
      (c: { occurrences?: number; evidence?: string[] }) => c.occurrences === (c.evidence?.length ?? 0)
    )
    ...
    record(
      ...
      report.phase === "proposals" &&
        report.recurring_classes?.length === 2 &&
        derivedOccurrences &&
        report.proposals?.length === 2 &&
        landings.join(",") === "method_block,settings" &&
        noCrs,
...
async function runFeatureCycleStages(temp: string): Promise<void> {
  ...
  git([
    ...
    "commit",
    "-qm",
    `spec evolution: add ${doc.docRel} (cycle ${cycleId})`,
  ])
  record(
    "feature-cycle: canonical evolved + committed (new doc joins the corpus)",
    existsSync(join(temp, doc.docRel)) && clean(),
    doc.docRel
  )
  ...
  record(
    ...
    result
      ? `${result.summary.split(":")[0]}; ${doc.docRel} in corpus, ${uncovered} uncovered; committed, clean tree`
      : failure || "extraction did not run"
  )
  ...
  record(
    ...
    Boolean(
      bumped &&
      fresh?.version === bumped &&
      result?.baselineId === `baseline-v${bumped}` &&
      fresh?.approval?.approval_ref === cycleId &&
      priorAfter?.superseded?.by_baseline_id === result?.baselineId
    ),
...
function authorEvolvedCorpus(temp: string, doc: { docRel: string; refs: string[]; title: string }, manifestPath: string): void {
  const manifest = readJson<{ baseline_id: string; version: string; manifest_hash: string; document_set_hash: string }>(
    join(temp, manifestPath)
  )
  ...
  const entry: CycleIssueEntry = {
    ...
    summary:
      "Implement the read-only build identifier that names the frozen baseline the build was produced from, stable within one baseline and changing only on a new freeze.",
  ...
  catalog.requirements.push({
    ...
    title: doc.title,
    statement:
      "The system exposes a read-only build identifier naming the frozen baseline it was built from; it changes only when a new baseline is frozen.",
    area: "feature-cycle",
    type: "functional",
    maturity: "mvp",
