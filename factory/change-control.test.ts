...
function fixture({ crs = {}, catalog, manifests }: { crs?: Record<string, Record<string, unknown>>; catalog?: unknown; manifests?: Record<string, unknown> } = {}) {
...
test("createChangeRequest rejects an invalid classification", () => {
  const { root, cleanup } = emptyRepo();
  try {
    assert.throws(() => createChangeRequest({ repoRoot: root, title: "x", classification: "nonsense" as CrClassification }), /invalid classification/);
  } finally {
    cleanup();
  }
});
