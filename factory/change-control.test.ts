...
import type { ChangeControlCheckResult, CrClassification } from "./change-control.ts"
...
function crBody(fm: Record<string, unknown>): string {
  const lines = Object.entries(fm).map(([k, v]) => `${k}: ${serialize(v)}`)
  return `---\n${lines.join("\n")}\n---\n\n# ${fm.id ?? "CR"}\n`
}

const validCr = (over: Record<string, unknown> = {}) => ({
  id: "CR-0001",
  title: "x",
  status: "idea",
...
function fixture({
  ...
  catalog,
  manifests,
}: { crs?: Record<string, Record<string, unknown>>; catalog?: unknown; manifests?: Record<string, unknown> } = {}) {
...
const has = (r: ChangeControlCheckResult, rule: string) => r.errors.some((e) => e.includes(rule))
    ...
    JSON.stringify(
      {
        ...
        version,
        status: "frozen",
        document_set_hash: `doc-${version}`,
        manifest_hash: `manifest-${version}`,
...
test("createChangeRequest rejects an invalid classification", () => {
  ...
  try {
    assert.throws(
      () => createChangeRequest({ repoRoot: root, title: "x", classification: "nonsense" as CrClassification }),
...
test("decideChangeRequest approved fills previous_* from the frozen manifest and passes the checker", () => {
  ...
  try {
    ...
    const result = decideChangeRequest({
      ...
      decision: "approved",
      decidedBy: "owner:ui",
      evidenceRef: "approved in the UI",
...
test("decideChangeRequest rejects an unknown CR id and a bad decision value", () => {
  ...
  try {
    assert.throws(
      () => decideChangeRequest({ repoRoot: root, id: "CR-9999", decision: "approved", decidedBy: "x" }),
      /no CR with id CR-9999/
    )
    createChangeRequest({ repoRoot: root, title: "x" })
    assert.throws(
      () => decideChangeRequest({ repoRoot: root, id: "CR-0001", decision: "maybe", decidedBy: "x" }),
