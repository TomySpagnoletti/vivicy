// Unit tests for the spike PROOFIER (S3 / G3), the substance-verification stage.
//
// BOTH agent legs are ALWAYS faked here — no real CLI is launched:
//   - the PROOFIER leg (spawnProofier) records the six evidence fields into the spike
...
import { flipSpikeStatus, runSpikeProofing } from "./spike-proofier.mjs";
...
function writeSpike(filename, { status = "pending", reqId = "REQ-ARCH-001", gated_by, blocks } = {}) {
  ...
  const content = [
    ...
    "",
    "```text",
    "environment: (to be filled by the proofier)",
    "```",
    "",
  ].join("\n");
...
/**
 * A fake PROOFIER: for the spike it is handed, it fills the six evidence fields into the
 * spike file (as the real proofier would) and writes the machine verdict per a lookup
 * keyed by gate_id (default `verified`). Records every call so ordering can be asserted.
 */
function fakeProofier(verdictByGate = {}, { onAttempt } = {}) {
  const calls = [];
  const spawnProofier = async (ctx) => {
    ...
    // so resolve the absolute path here — the real proofier likewise edits the file in-repo.
...
describe("runSpikeProofing — agree + verified", () => {
  it("flips the spike's traceability status to verified IN the file (no folder move)", async () => {
    ...
    const { spawnProofier } = fakeProofier({ [s.gate_id]: "verified" });
    const { spawnProofVerifier } = fakeProofVerifier({ [s.gate_id]: true });

    const result = await runSpikeProofing({ repoRoot: temp, spawnProofier, spawnProofVerifier });

    assert.deepEqual(
      result.proofed.map((p) => [p.gate_id, p.verdict]),
      [[s.gate_id, "verified"]],
    );
    ...
    // The verified spike, with the proofier's evidence, is a spike-check-clean corpus.
  ...
  it("emits spike_proof_started/completed through the injected recordEvent sink", async () => {
    ...
    await runSpikeProofing({ repoRoot: temp, spawnProofier, spawnProofVerifier, recordEvent: (e) => events.push(e), now: () => "2026-07-02T00:00:00.000Z" });
    ...
    assert.equal(events[0].role, "spike_proofier");
  ...
  it("does not throw when recordEvent is null (the extraction path passes null)", async () => {
    ...
    const { spawnProofier } = fakeProofier({ [s.gate_id]: "verified" });
    const { spawnProofVerifier } = fakeProofVerifier({ [s.gate_id]: true });
    const result = await runSpikeProofing({ repoRoot: temp, spawnProofier, spawnProofVerifier, recordEvent: null });
    assert.equal(result.proofed.length, 1);
  });
});

describe("runSpikeProofing — agree + failed drafts a Change Request", () => {
  it("flips the spike to failed and writes a CR whose frontmatter passes change-control", async () => {
    const s = writeSpike("01-provider-auth.md");
    const { spawnProofier } = fakeProofier({ [s.gate_id]: "failed" });
    const { spawnProofVerifier } = fakeProofVerifier({ [s.gate_id]: true }); // agrees it failed

    const result = await runSpikeProofing({ repoRoot: temp, spawnProofier, spawnProofVerifier });

    assert.equal(result.proofed.length, 0);
    assert.deepEqual(
    ...
    assert.match(crText, new RegExp(proofRel(s.file).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")), "CR cites the proofier report");
  ...
  it("numbers a second drafted CR sequentially (CR-0002) so change-control stays gap-free", async () => {
    ...
    const { spawnProofier } = fakeProofier({ [a.gate_id]: "failed", [b.gate_id]: "failed" });
...
describe("runSpikeProofing — disagreement retries once then drafts a CR", () => {
  it("retries the pair once feeding the disagreement back, then flips failed + CR on persistent disagreement", async () => {
    const s = writeSpike("01-provider-auth.md");
    // Proofier always claims verified; verifier always disagrees -> never resolves.
    const { spawnProofier, calls: proofCalls } = fakeProofier({ [s.gate_id]: "verified" });
    ...
    assert.equal(proofCalls.length, 2, "proofier ran twice (bounded retry)");
    assert.equal(verifyCalls.length, 2, "verifier ran twice");
    // The retry carried the disagreement back to the proofier.
  ...
  it("a disagreement RESOLVED on the retry flips verified with no CR", async () => {
    ...
    assert.equal(readStatus(s.file), "verified");
    assert.deepEqual(result.proofed.map((p) => p.gate_id), [s.gate_id]);
    assert.equal(result.changeRequests.length, 0, "a resolved disagreement drafts no CR");
  });
});

describe("runSpikeProofing — topological ordering of the gated_by graph", () => {
  it("proofs a gate BEFORE the spike it gates (a spike waits for its gated_by within the run)", async () => {
    // 01-a is gated_by 02-b: 02-b must be proofed FIRST, then 01-a.
    ...
    const { spawnProofier, calls: proofCalls } = fakeProofier({ [a.gate_id]: "verified", [b.gate_id]: "verified" });
    const { spawnProofVerifier } = fakeProofVerifier({ [a.gate_id]: true, [b.gate_id]: true });

    const result = await runSpikeProofing({ repoRoot: temp, spawnProofier, spawnProofVerifier });

    // Both verified, and b (the gate) was proofed before a (the dependent).
    const order = proofCalls.map((c) => c.gate_id);
    assert.deepEqual(order, [b.gate_id, a.gate_id], "the gate is proofed before its dependent");
    ...
    assert.equal(result.proofed.length, 2);
  });

  it("skips a spike whose gate FAILED this run (never proofs on an unproven foundation)", async () => {
    const a = writeSpike("01-a.md", { gated_by: "gate:phase0:s02-b" });
    const b = writeSpike("02-b.md", { reqId: "REQ-ARCH-002", blocks: "gate:phase0:s01-a" });
    // b fails; a must then be SKIPPED (its gate is not verified), not proofed.
    const { spawnProofier, calls: proofCalls } = fakeProofier({ [a.gate_id]: "verified", [b.gate_id]: "failed" });
    const { spawnProofVerifier } = fakeProofVerifier({ [a.gate_id]: true, [b.gate_id]: true });

    const result = await runSpikeProofing({ repoRoot: temp, spawnProofier, spawnProofVerifier });

    // Only b ran through the proofier; a was skipped before spawning any leg.
  ...
  it("skips a spike gated by an already-failed spike ON DISK (not proofed this run)", async () => {
    ...
    const { spawnProofier, calls: proofCalls } = fakeProofier({ [a.gate_id]: "verified" });
  ...
  it("leaves an already-verified spike alone (only pending spikes are proof candidates)", async () => {
    ...
    const { spawnProofier, calls: proofCalls } = fakeProofier();
    const { spawnProofVerifier } = fakeProofVerifier();
    const result = await runSpikeProofing({ repoRoot: temp, spawnProofier, spawnProofVerifier });
    assert.equal(proofCalls.length, 0, "a verified spike is not re-proofed");
    assert.equal(result.proofed.length, 0);
    assert.equal(result.failed.length, 0);
    assert.equal(readStatus(s.file), "verified");
  });
...
describe("runSpikeProofing — honest failure on a dead/timed-out leg", () => {
  it("a TIMED-OUT proofier leg (writes no report) is an honest failed proof + CR, never a silent green", async () => {
    const s = writeSpike("01-provider-auth.md");
    // The proofier "leg" mimics a leg-timeout kill: it writes NO verdict JSON and carries a timeout result.
    const spawnProofier = async () => ({
    ...
    assert.equal(result.proofed.length, 0, "a timed-out proof is never verified");
    assert.deepEqual(result.failed.map((f) => f.gate_id), [s.gate_id]);
  ...
  it("a proofier that says verified but writes NO report is not trusted (no report -> failed)", async () => {
    const s = writeSpike("01-provider-auth.md");
    // Proofier fills evidence and returns a clean exit but writes NO verdict JSON at all.
    const spawnProofier = async (ctx) => {
      fillEvidence(resolve(temp, ctx.spike.file));
    ...
    assert.equal(result.proofed.length, 0, "a missing proof report is never a green");
    assert.equal(readStatus(s.file), "failed");
    assert.equal(result.changeRequests.length, 1);
  });
});

describe("runSpikeProofing — nothing to do", () => {
  it("returns empty results and spawns no leg when there are no spikes", async () => {
    ...
    const result = await runSpikeProofing({ repoRoot: temp, spawnProofier, spawnProofVerifier });
    assert.deepEqual(result, { proofed: [], failed: [], skipped: [], changeRequests: [] });
