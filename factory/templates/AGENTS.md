...
- `gateCommand` (required) — the command Vivicy runs as the per-issue verification gate and re-runs itself as the authoritative verdict on every issue. Set it to this project's real test runner (for example `go test ./...`, `cargo test`, `pytest -q`, `phpunit`, `swift test`, or `npm test`). An issue may override it via its own `gate_command`. A field explicitly present in `vivicy.json`, even an empty array, always wins over the `package.json#vivicy` fallback for that field.
