...
const COMPLETION_FIELDS = ["environment", "commands", "observed output", "decision", "documentation updates", "unresolved risks"]
...
function validateSpikeGatingGraph(spikes: Spike[], fail: FailFn): void {
  ...
  for (const spike of spikes) {
    ...
    for (const g of spike.gated_by) {
      if (!byGate.has(g)) {
        fail(
          ...
          label,
          `gated_by "${g}" matches no spike`,
          "every gated_by entry is another spike's gate_id",
          `fix gated_by in ${label}`
        )
      }
    }
    for (const b of spike.blocks) {
      if (!byGate.has(b)) {
        fail(
          "spike_blocks_resolves",
          label,
          `blocks "${b}" matches no spike`,
          "every blocks entry is another spike's gate_id",
          `fix blocks in ${label}`
        )
        ...
        fail(
          ...
          label,
          `${spike.gate_id} blocks ${b}, but ${b} does not list it in gated_by`,
          "a blocks edge is mirrored by the target's gated_by",
          `add gated_by: ${spike.gate_id} to ${b}, or remove blocks in ${label}`
        )
      }
    }
  }
  ...
  if (cycle) {
    fail(
      ...
      SPIKES_DIR,
      `gated_by cycle: ${cycle.join(" -> ")}`,
      "the gated_by graph is acyclic (like issue depends_on)",
      "break the cycle in the spikes' gated_by lists"
    )
  }
  ...
  for (const spike of spikes) {
    ...
    if (unmet.length > 0) {
      fail(
        ...
        spike.file,
        `${spike.gate_id} is verified but its gated_by chain is not verified: ${unmet.join(", ")}`,
        "a spike is verified only after every spike that gates it is verified",
        `verify ${unmet.join(", ")} first, or downgrade ${spike.gate_id}`
      )
    }
  }
}
...
function done(errors: string[], scope = ""): SpikeCheckResult {
  return {
    ...
    errors,
    placeholder: false,
    summary: errors.length > 0 ? `spike-check: FAILED with ${errors.length} error(s)` : `spike-check: OK${scope ? ` (${scope})` : ""}`,
  }
}
