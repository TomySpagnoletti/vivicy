/**
 * Filesystem reader that reproduces `factory/dev-status.mjs`'s *deterministic*
 ...
 * dev-status.mjs (which additionally inspects live processes).
 *
...
export function readDevStatusFromDisk(root: string): DevStatus {
  ...
  // Per-agent quota state, mirroring dev-status.mjs. Absent => unknown (no
