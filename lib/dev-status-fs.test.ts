...
describe("readDevStatusFromDisk — verdict ordering", () => {
  it("(1) DONE when every issue is done via a done/ file", () => {
    writeIndex({ issues: [{ id: "ISSUE-1" }, { id: "ISSUE-2" }] })
    writeDoneFiles(["ISSUE-1", "ISSUE-2"])
    ...
    expect(status.done).toEqual(["ISSUE-1", "ISSUE-2"])
    expect(status.remaining).toEqual([])
  })

  it("(2) STOPPED (last gate failed) when some are done AND a gate is failing", () => {
    writeIndex({ issues: [{ id: "ISSUE-1" }, { id: "ISSUE-2" }, { id: "ISSUE-3" }] })
    writeDoneFiles(["ISSUE-1"])
    ...
    expect(status.remaining).toEqual(["ISSUE-2", "ISSUE-3"])
  })
  ...
  it("(4) NOT STARTED when nothing is done", () => {
    ...
    expect(status.remaining).toEqual(["ISSUE-1", "ISSUE-2"])
  })
...
describe("readDevStatusFromDisk — per-issue done rule", () => {
  it("(5) counts an issue done when EVERY graph_ref is verified in the ledger (no done/ file)", () => {
    writeIndex({
      issues: [
        { id: "ISSUE-1", graph_refs: ["ref-a", "ref-b"] },
        { id: "ISSUE-2", graph_refs: ["ref-c"] },
      ],
    })
    writeLedger({
      graph_item_states: [
        { graph_ref: "ref-a", issue_states: { "ISSUE-1": "verified" } },
        { graph_ref: "ref-b", issue_states: { "ISSUE-1": "verified" } },
        { graph_ref: "ref-c", issue_states: { "ISSUE-2": "in_progress" } },
      ],
    })

    const status = readDevStatusFromDisk(root)
    expect(status.done).toEqual(["ISSUE-1"])
    expect(status.remaining).toEqual(["ISSUE-2"])
    expect(status.issues_done).toBe(1)
    expect(status.verdict).toBe("STOPPED (resume to continue)")
  })
  ...
  it("does NOT count an issue with an empty graph_refs list as done via the ledger (refs.length>0 guard)", () => {
    writeIndex({ issues: [{ id: "ISSUE-1", graph_refs: [] }] })
  ...
  it("a done/ file wins even when the ledger shows the issue unverified", () => {
    writeIndex({ issues: [{ id: "ISSUE-1", graph_refs: ["ref-a"] }] })
    writeDoneFiles(["ISSUE-1"])
    writeLedger({
      graph_item_states: [{ graph_ref: "ref-a", issue_states: { "ISSUE-1": "in_progress" } }],
    })
  ...
  it("ignores non-.md entries in the done/ dir", () => {
    ...
    const doneDir = path.join(devDir(), "issues", "done")
    mkdirSync(doneDir, { recursive: true })
    writeFileSync(path.join(doneDir, "ISSUE-1.txt"), "not a done marker")
  ...
  it("surfaces active_items verbatim from the ledger", () => {
    writeIndex({ issues: [{ id: "ISSUE-1" }] })
    writeLedger({ active_items: [{ id: "ISSUE-1", actor: "claude" }] })

    const status = readDevStatusFromDisk(root)
    expect(status.active).toEqual([{ id: "ISSUE-1", actor: "claude" }])
  })
})
