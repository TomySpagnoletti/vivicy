...
describe("governance guard", () => {
  it("refuses a target that is already governed, before any write", async () => {
    ...
    await expect(startGovernance({ targetDir: target, entries: [fileEntry("a.md", "# hi\n")] })).rejects.toMatchObject({
  ...
  it("governs and imports into a brand-new (non-existent) directory", async () => {
    ...
    const result = await startGovernance({ targetDir: target, entries: [fileEntry("spec.md", ENGLISH)] })

    expect(isGovernedRoot(result.project.root)).toBe(true)
    ...
    expect(result.batch?.accepted.map((f) => f.path)).toEqual(["spec.md"])
    expect(existsSync(path.join(result.project.root, UPLOADS_DIR, result.batch!.batchId, "spec.md"))).toBe(true)
  })
  ...
  it("governs and imports into an existing non-governed directory without clobbering its files", async () => {
    ...
    const result = await startGovernance({ targetDir: target, entries: [fileEntry("docs/intro.md", ENGLISH)] })
    ...
    expect(existsSync(path.join(result.project.root, UPLOADS_DIR, result.batch!.batchId, "docs", "intro.md"))).toBe(true)
  })

  it("lays the skeleton exactly once — a second acquisition is refused as already_governed", async () => {
    const target = targetPath("once")
    await startGovernance({ targetDir: target, entries: [fileEntry("a.md", ENGLISH)] })
    await expect(startGovernance({ targetDir: target, entries: [fileEntry("b.md", ENGLISH)] })).rejects.toMatchObject({
      code: "already_governed",
    })
  })
})

describe("govern-only (zero documents)", () => {
  it("governs a new directory with no batch when no documents are supplied", async () => {
    const target = targetPath("govern-fresh")
    const result = await startGovernance({ targetDir: target, entries: [] })
  ...
  it("governs an existing empty directory (govern-only)", async () => {
    const target = targetPath("govern-empty")
    mkdirSync(target, { recursive: true })
    const result = await startGovernance({ targetDir: target, entries: [] })
  ...
  it("governs a brownfield code directory in place, leaving its files byte-identical", async () => {
    ...
    for (const [rel, contents] of Object.entries(before)) {
      writeFileSync(path.join(target, ...rel.split("/")), contents)
    }
  ...
  it("derives the project name from the target basename, and honors an explicit name", async () => {
    const derived = await startGovernance({ targetDir: targetPath("my-service"), entries: [] })
    expect(derived.project.name).toBe("my-service")

    const named = await startGovernance({ targetDir: targetPath("api"), projectName: "Billing API", entries: [] })
    expect(named.project.name).toBe("api")
    expect(existsSync(path.join(named.project.root, "AGENTS.md"))).toBe(true)
    expect(readFileSync(path.join(named.project.root, "AGENTS.md"), "utf8")).toContain("Billing API")
  })
})
...
describe("batch layout and ids", () => {
  it("writes the batch under .vivicy/uploads/<id>/ preserving nested structure", async () => {
    ...
    const batchDir = path.join(result.project.root, UPLOADS_DIR, result.batch!.batchId)
...
describe("per-file type filtering", () => {
  it("accepts supported types and rejects unknown ones per-file, not the whole batch", async () => {
    ...
    expect(result.batch?.accepted.map((f) => f.path).sort()).toEqual(["data.csv", "good.md"])
    expect(result.batch?.rejected).toEqual([{ path: "bad.exe", code: "unsupported_type" }])
  })
...
describe("zip explosion", () => {
  it("explodes a zip into the batch preserving its internal structure", async () => {
    ...
    expect(result.batch?.accepted.map((f) => f.path).sort()).toEqual(["b.txt", "canon/a.md"])
    expect(result.batch?.rejected).toEqual([{ path: "skip.exe", code: "unsupported_type" }])
    const batchDir = path.join(result.project.root, UPLOADS_DIR, result.batch!.batchId)
  ...
  it("rejects a zip-slip (../) entry as a security boundary", async () => {
    ...
    await expect(
      startGovernance({ targetDir: targetPath("slip"), entries: [{ rel: "evil.zip", name: "evil.zip", bytes: zip }] })
  ...
  it("explodes one level of nesting but rejects a zip nested deeper", async () => {
    ...
    expect(result.batch?.accepted.map((f) => f.path)).toEqual(["deep.md"])
    expect(result.batch?.rejected).toEqual([{ path: "toodeep.zip", code: "unsupported_type" }])
  })
})
...
describe("manifest", () => {
  it("records relative path, byte size, and sha256 for every batch file, sorted", async () => {
    ...
    const manifest = readManifest(result.project.root, result.batch!.batchId)
    expect(manifest.batchId).toBe(result.batch!.batchId)
  ...
  it("detects the language of a binary .docx batch deterministically, without any leg", async () => {
    ...
    expect(result.batch?.language).toBe("fra")
    expect(readManifest(result.project.root, result.batch!.batchId).language).toBe("fra")
  })
...
describe("cycle binding", () => {
  it("binds a first import to the project cycle (pre-freeze, no baseline, no spec cycle)", async () => {
    const result = await startGovernance({ targetDir: targetPath("bind-project"), entries: [fileEntry("a.md", ENGLISH)] })
    expect(result.batch?.cycle).toEqual({ binding: "active", id: "project" })
    expect(readManifest(result.project.root, result.batch!.batchId).cycle).toEqual({ binding: "active", id: "project" })
  })
...
async function governedRoot(name: string): Promise<string> {
  return (await startGovernance({ targetDir: targetPath(name), entries: [fileEntry("seed.md", ENGLISH)] })).project.root
}
