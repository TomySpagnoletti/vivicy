...
function subscribeStore(key: string, defaultValue: boolean, listener: () => void): () => void {
...
function writeStore(key: string, defaultValue: boolean, action: SetStateAction<boolean>): void {
  const store = storeFor(key, defaultValue)
...
export function usePersistedBoolean(key: string, defaultValue: boolean): [boolean, Dispatch<SetStateAction<boolean>>] {
  const subscribe = useCallback((listener: () => void) => subscribeStore(key, defaultValue, listener), [defaultValue, key])
  const getSnapshot = useCallback(() => snapshotStore(key, defaultValue), [defaultValue, key])
  const getServerSnapshot = useCallback(() => defaultValue, [defaultValue])

  const value = useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot)

  const setValue = useCallback<Dispatch<SetStateAction<boolean>>>((action) => writeStore(key, defaultValue, action), [defaultValue, key])
