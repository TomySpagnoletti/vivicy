...
export async function POST(request: Request) {
  try {
    ...
    const created = createDirectory(typeof body?.parent === "string" ? body.parent : null, body?.name)
    ...
    return Response.json({ ok: false, error: error instanceof Error ? error.message : "failed to create directory" }, { status: 500 })
  }
}
